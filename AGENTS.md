...
AGENTS.md                                    This file
.Codex/commands/infographic.md              `/infographic` command definition (site pipeline)
...
Pipeline (per unit, codified in `.Codex/commands/infographic.md`):
...
- **`Codex.ai Figma`** — design-to-code, screenshots, FigJam diagrams, Code Connect. Use for Figma URLs (parse `fileKey` and `nodeId`; convert `-` → `:`).
...
**Local build:**
```bash
cd /Users/artemmac/dev/Skein/site
bun install
