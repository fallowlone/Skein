...
async function setup({ coach = true, apiKey = "sk-server", limit = "2" } = {}) {
  ...
  if (coach) await setEntitlement(db as any, user.id, "coach", true, "test", "S_1", 1);
  const env = { DB: db, TERMS_VERSION: "v1", ANTHROPIC_API_KEY: apiKey, COACH_AI_MONTHLY_REQUESTS: limit } as any;
  return { db, user, env };
}
...
describe("managed Coach grading", () => {
  it("blocks non-entitled users before any provider call", async () => {
    ...
    const r = await onRequestPost({ request: request(), env, data: { userId: user.id } } as any);
  ...
  it("returns 503 without a managed provider key and never fakes a result", async () => {
    const { env, user } = await setup({ apiKey: "" });
    const provider = vi.spyOn(globalThis, "fetch");
    const r = await onRequestPost({ request: request(), env, data: { userId: user.id } } as any);
  ...
  it("rejects null and oversized streaming JSON before any provider call", async () => {
    ...
    const provider = vi.spyOn(globalThis, "fetch");
    const nullResponse = await onRequestPost({ request: rawRequest("null"), env, data: { userId: user.id } } as any);
    ...
    const oversizedResponse = await onRequestPost({ request: oversized, env, data: { userId: user.id } } as any);
    expect(oversizedResponse.status).toBe(413);
    expect(provider).not.toHaveBeenCalled();
  });

  it("enforces the monthly hard limit before making another provider request", async () => {
    const { env, user } = await setup({ limit: "1" });
    ...
    const first = await onRequestPost({ request: request(), env, data: { userId: user.id } } as any);
    expect(first.status).toBe(200);
    const second = await onRequestPost({ request: request(), env, data: { userId: user.id } } as any);
    expect(second.status).toBe(429);
    expect(provider).toHaveBeenCalledTimes(1);
  });

  it("refunds a credit when the provider rejects the request", async () => {
    const { db, env, user } = await setup({ limit: "1" });
