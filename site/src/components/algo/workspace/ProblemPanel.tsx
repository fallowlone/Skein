...
type Props = { lang: Locale; labels: Labels; problem: WorkspaceProblem; submitted: boolean };

export default function ProblemPanel({ lang, labels, problem, submitted }: Props) {
    ...
    <section style="border-right:0.5px solid var(--rule);padding:32px;display:flex;flex-direction:column;gap:24px;min-width:0">
      ...
      {problem.followUp && (
        <div style="margin-top:auto;border-top:0.5px solid var(--rule);padding-top:14px">
          ...
          <p style="margin:0;font-size:13.5px;line-height:1.6;color:var(--muted);text-wrap:pretty">
            {submitted
              ? problem.followUp[lang]
              : lang === "ru"
                ? "Отправь текущую попытку, чтобы открыть дополнительный вопрос."
                : "Submit your current attempt to unlock the follow-up question."}
          </p>
        </div>
      )}
    </section>
