...
describe("cross-track-edges source validation", () => {
  ...
  it("accepts a valid cross-track edge", () => {
    const errs = validatePathData({ ...base, crossTrackEdges: [{ concept: "a", requires: "b" }] } as unknown as PathData);
    expect(errs.filter((e) => e.includes("cross-track-edges"))).toEqual([]);
  });
  it("flags an unknown id", () => {
    const errs = validatePathData({ ...base, crossTrackEdges: [{ concept: "a", requires: "ghost" }] } as unknown as PathData);
    expect(errs.some((e) => e.includes("cross-track-edges") && e.includes("ghost"))).toBe(true);
  });
  it("flags an intra-track edge", () => {
    const errs = validatePathData({ ...base, crossTrackEdges: [{ concept: "a", requires: "c" }] } as unknown as PathData);
