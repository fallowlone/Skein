...
[Violation] Forced reflow while executing JavaScript took 51 ms
    ...
    />
    <p><strong>A virtualisation design challenge.</strong> The most common render pipeline failure in production is virtualised lists. Implement scrolling a 50 000-message chat history at 60 fps on mobile with a 16.67 ms frame budget. Constraints: layout must not measure off-screen rows, layer count capped at 30, no forced reflow in the scroll handler. Solution architecture: render only the visible rows plus a small overscan buffer; position them with transform: translateY (compositor-only). Scroll handler reads scrollTop once, computes which rows are visible via integer math (no DOM measurements), and updates the React/Preact state. Batch DOM reads before writes to prevent forced reflow. Will-change: transform is scoped to active time only. On mobile, this is the difference between silky 60 fps scroll and 20 fps jank.</p>
  </Fragment>
</TierAccordion>
