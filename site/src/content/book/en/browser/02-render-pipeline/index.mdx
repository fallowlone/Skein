...
**Myth:** rAF batches everything — wrap a read-then-write loop in `requestAnimationFrame` and layout will be efficient.

**Reality:** rAF batches only the *invocation*. The moment any code reads a layout-dependent value (`offsetWidth`, `getBoundingClientRect`, `scrollTop`) after a write, the browser flushes layout synchronously. The fix is separating reads from writes — not wrapping in rAF.
</Misconception>
