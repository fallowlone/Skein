...
import fanoutAmplification from "./infographics/fanout-amplification.svg?raw";
...
<Infographic
  svg={fanoutAmplification}
  label="Column chart of feed writes triggered by a single post under pure fan-out-on-write: a normal author with ~200 followers causes 200 writes, an active user with 5,000 followers causes 5,000, and a celebrity with 30M followers causes 30,000,000."
  caption="One post, three regimes: push costs a few hundred writes for a normal author but 30 million for a celebrity — five orders of magnitude. That tail is why no single fan-out strategy works, and why the hybrid pulls celebrities at read time."
  hue="var(--d-infra)"
/>
...
<TradeoffMatrix
  ...
  lang="en"
  prompt="Your social feed is 100:1 read-heavy (35,000 feed reads/sec vs 7,000 posts/sec). Most users have under 5,000 followers; a small number of celebrities have 30M+ followers. Feed reads must return in under 100 ms. Which fan-out strategy fits?"
  options={[
    { name: "Hybrid: push for normal authors, pull for celebrities", summary: "Cheap reads for everyone, no write storm from celebrities — matches each strategy to the regime where it is cheap.", justification: "Push (fan-out-on-write) precomputes a per-user feed cache by inserting post IDs on write. For a user with 5,000 followers that is 5,000 cheap writes — amortized over the 100× more reads, it's the clear win. For a celebrity with 30M followers, push is 30M writes per post — catastrophic. The fix: classify accounts above a follower threshold as 'pull'; at read time, the service fetches that celebrity's recent posts directly and merges them into the mostly-precomputed feed. The merge is cheap because a user follows few celebrities. This is the only strategy that satisfies all constraints.", correct: true },
    { name: "Pure fan-out-on-write (push for everyone)", summary: "Fastest reads — but a celebrity post causes tens of millions of writes and stalls everyone's feed.", justification: "Pure push gives the cheapest reads (a feed read is just 'fetch the precomputed list') and fully amortizes write work over many reads — ideal for the 100:1 ratio. But the celebrity tail breaks it: a single 30M-follower post triggers 30M feed insertions synchronously, the fan-out workers queue for hours, and the feed goes stale for users who follow celebrities. The hook's outage is exactly this. Push is correct for the normal case; it needs pull as an escape hatch for the tail." },
    { name: "Pure fan-out-on-read (pull for everyone)", summary: "Trivial writes — but every feed load is a scatter-gather across all followed accounts, with no amortization.", justification: "Pull stores each post once (by author) and assembles the feed at read time by querying recent posts from every followed account. Writes are cheap regardless of follower count. But at 100:1 read-heavy, every one of the 35,000/sec feed reads triggers a multi-author scatter-gather: for a user following 1,000 accounts, that is 1,000 parallel queries, merged and ranked, on every refresh. At this scale and latency target (<100 ms), pure pull is infeasible. Pull is only the right strategy when writes vastly outnumber reads — the opposite of a feed." },
    { name: "Precompute all feeds nightly in a batch job", summary: "Reads are fast but feeds are hours stale — unacceptable for a live social product.", justification: "Batch precomputation could produce fast reads, but a feed computed at 2 am and served until 2 am the next day misses every post published today. Social feeds need near-real-time freshness (seconds to a minute for new posts). Batch is the correct pattern for reports and aggregations over stable data, not for a product where the defining value is seeing what your connections just shared." },
  ]}
/>
