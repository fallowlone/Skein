...
<Infographic
  svg={latencyBudget}
  label="A funnel of the ~100 ms end-to-end autocomplete budget narrowing through each stage: the client debounce holds for tens of ms, the network round trip eats a big chunk, leaving the server only tens of ms in which the precomputed trie lookup must fit."
  caption="The budget is consumed stage by stage; the precomputed trie lookup has to fit in the small slice the server is left with — which is exactly why ranking can never happen on the read path."
  hue="var(--d-infra)"
/>
