...
import freshnessVsCost from "./infographics/freshness-vs-cost.svg?raw";
...
<Infographic
  svg={freshnessVsCost}
  label="A two-column comparison of ping interval: pinging every 1 second produces about 10 million writes per second — roughly quadrupling write load for only marginal extra freshness — while pinging every 5 seconds is the design's 2 million writes-per-second baseline, lets the rate adapt down when a user is stationary, and self-heals since a dropped ping is replaced within seconds."
  caption="The whole write firehose is set by the ping interval, not by message size — so an adaptive rate (slow when still, fast when moving) buys most of the realtime feel for a fraction of the writes."
  hue="var(--d-infra)"
/>
