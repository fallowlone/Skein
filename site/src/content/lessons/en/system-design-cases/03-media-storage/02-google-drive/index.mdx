...
<Infographic
  ...
  label="Column chart comparing bytes moved per device for a one-paragraph edit to a 200 MB file: a naive full re-upload moves 200 MB, while delta sync moves only the 1–2 changed chunks (~8 MB)."
  caption="Content-addressed chunking turns a 200 MB re-upload into an ~8 MB chunk transfer — a 25× bandwidth win on every small edit, multiplied across every device that syncs."
  hue="var(--d-infra)"
/>
