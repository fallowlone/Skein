...
<Infographic
  ...
  label="Side-by-side comparison of email as four stacked problems handled naively versus the scaled design: full attachment copy per recipient (50M copies = 100 TB) versus a deduplicated ref-counted blob with 50M pointers; scanning 200K messages per query versus a per-user inverted index answering in tens of ms; re-listing the whole mailbox to sync (O(mailbox)) versus a per-mailbox change log (O(changes)); storing mail then filtering spam (~10x extra) versus rejecting ~90% spam at SMTP ingest before storage."
  caption="Email is four stacked problems — attachments, search, sync, spam — and each flips from ruinous to affordable with one design move: dedup the bytes, index per user, sync by change log, reject spam at ingest."
  hue="var(--d-infra)"
/>
