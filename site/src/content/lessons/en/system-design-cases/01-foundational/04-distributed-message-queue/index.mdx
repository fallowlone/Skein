...
import acksDurabilityLadder from "./infographics/acks-durability-ladder.svg?raw";
...
<Infographic
  svg={acksDurabilityLadder}
  label="An ascending three-step ladder of the producer acks setting: acks=0 (fire-and-forget, fastest, can lose data), acks=1 (leader-only acknowledgement, loses data if the leader dies before followers replicate), and acks=all (every in-sync replica must have the message, survives leader failure with no loss)."
  caption="The acks knob is a durability-for-speed ladder: acks=0 never waits but can lose data, acks=1 trusts the leader alone, and acks=all waits for the full ISR so no committed write is lost."
  hue="var(--d-infra)"
/>
...
<TradeoffMatrix
  id="04-distributed-message-queue-tradeoff"
  lessonSlug="04-distributed-message-queue"
  lang="en"
  prompt="A payment-event consumer reads messages from a Kafka partition and calls a downstream ledger service on each. The consumer crashes mid-batch. Which delivery semantic and offset-commit strategy should you choose?"
  options={[
    { name: "At-least-once (commit after processing)", summary: "Commit the offset after processing; on crash, re-read and reprocess.", justification: "Committing the offset only after the ledger call succeeds means a consumer crash restarts from the last committed offset and reprocesses the unacknowledged messages. No payment event is ever skipped. The cost is that the consumer must be idempotent: each message carries a payment_id and the ledger rejects a second write for the same id. This is the standard production answer for financial systems: at-least-once delivery paired with an idempotent consumer gives effectively-once semantics as the user experiences it, without requiring the additional coordinator overhead of Kafka transactions.", correct: true },
    { name: "At-most-once (commit before processing)", summary: "Commit the offset before processing; on crash, the message is skipped.", justification: "Committing the offset before calling the ledger means a crash between commit and completion silently drops the payment event. On restart the consumer picks up from the committed (already-advanced) position and never reprocesses the lost message. This is acceptable for low-stakes metrics pipelines where an occasional missed event is tolerable, but it is the wrong choice for a payment ledger: a skipped message means money that was charged but never recorded, discovered only in a later reconciliation batch. The simplicity gain does not outweigh the correctness risk for financial data." },
    { name: "Exactly-once (Kafka transactions)", summary: "Use Kafka transactional producer/consumer to atomically commit offset and effect.", justification: "Kafka's transactional API allows atomically committing a consumer offset and a producer write in the same transaction, achieving true exactly-once if both sides are Kafka topics. For a payment ledger backed by an external database the boundary extends outside Kafka: the transaction cannot span the Kafka offset commit and the database write atomically without a two-phase commit. Most real systems achieve the same outcome more simply with at-least-once delivery plus an idempotent consumer that deduplicates on payment_id. Kafka transactions are the right tool when the side effect is itself a Kafka topic and both endpoints are Kafka-native." },
  ]}
/>
