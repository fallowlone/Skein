...
<Infographic
  svg={lostUpdateFixes}
  label="A list of the three concurrency-control fixes for the lost update: pessimistic lock (SELECT FOR UPDATE) blocks the second debit but serializes hot wallets; optimistic/CAS writes only if the version is unchanged and retries on conflict, ideal under low contention but storming under high; and the atomic conditional write pushes check-and-decrement into one UPDATE WHERE balance >= amount statement, the cleanest single-wallet debit."
  caption="All three close the stale-read-then-write gap — what differs is how a hot wallet behaves: a pessimistic lock queues it, CAS storms with retries, and the atomic conditional write decides-and-writes in one indivisible statement."
  hue="var(--d-infra)"
/>
