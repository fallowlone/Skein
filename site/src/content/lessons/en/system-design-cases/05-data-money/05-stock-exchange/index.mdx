...
import singleSequenceTradeoff from "./infographics/single-sequence-tradeoff.svg?raw";
...
<Infographic
  svg={singleSequenceTradeoff}
  label="Side-by-side comparison of two architectural stances. Left, distribute for scale (the unit's instinct): parallel threads on shared state, order decided by lock scheduling, replicas can disagree on who filled first, scale by adding threads. Right, funnel through one sequencer (an exchange): a single-threaded matching core with order fixed by global sequence number, every replica and replay agrees, and you scale the one thread and partition by instrument."
  caption="An exchange inverts the unit's distribute-for-scale playbook: fairness forces one total order, so it funnels through a single sequencer and a single-threaded core for a deterministic, auditable sequence — and scales the thread, not the book."
  hue="var(--d-infra)"
/>
