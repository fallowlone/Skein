...
<Infographic
  ...
  label="Column chart of active time-series count: service×status×region with bounded labels is 2,000 series, but adding one unbounded user_id label with a million values multiplies it to 2,000,000,000 series."
  caption="Series count is the product of label cardinalities, so one unbounded label doesn't add — it multiplies a healthy 2,000 series into 2 billion, blowing the in-memory index past RAM. That multiplicative jump is the entire reason cardinality, not sample rate, kills the TSDB."
  hue="var(--d-infra)"
/>
