...
<Infographic
  svg={speedVsBatch}
  label="Side-by-side comparison of the two lambda layers: the speed (streaming) layer is seconds-fresh, approximate, self-healing and watermark-pressured; the batch (reconciliation) layer is hours-late, authoritative, the source of truth, and runs with no time pressure over all raw events."
  caption="Same source of truth, opposite tradeoffs: the speed layer trades exactness for freshness so the dashboard is fast, while the batch layer trades freshness for exactness so the billing number is right."
  hue="var(--d-infra)"
/>
