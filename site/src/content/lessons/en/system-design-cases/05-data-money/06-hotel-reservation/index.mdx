...
<Infographic
  ...
  label="Two-way comparison of the two paths: the availability search path handles ~10,000 searches/sec, is eventually consistent, served from caches and replicas, advisory and slightly stale; the reservation path handles ~100-1,000 bookings/sec, is strongly consistent on a single shard per hotel, does an atomic all-or-nothing check-and-decrement across every night, and is the only real guarantee against double-booking."
  caption="The whole architecture is one split: search dominates traffic (~100x) and can be stale and advisory, while the tiny reservation path must be strongly consistent and atomic — that opposite-consistency contrast is the design."
  hue="var(--d-infra)"
/>
