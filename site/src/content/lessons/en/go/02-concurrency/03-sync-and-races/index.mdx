...
<Crux>No happens-before edge, no guarantee: racy programs have undefined behavior, even "harmless" flag reads. Synchronize via channels/mutexes/atomics and run -race in CI.</Crux>

<Explanation>

## Happens-before: the only ordering that exists

The Go memory model (`go.dev/ref/mem`) defines when a read is *guaranteed* to observe a write: only when the write **happens-before** the read. Within one goroutine, program order gives you that for free. *Between* goroutines, happens-before is created exclusively by synchronization:
