...
<Crux>Goroutines are ~2 KB growable stacks scheduled by Go, not the OS. GMP multiplexes them onto threads; cheap to start, easy to leak — pprof counts the zombies.</Crux>
...
<FlowDiagram
  label="A new goroutine flows into a P's local run queue, an M executes it, and idle Ps steal work while syscalls hand the P to another thread"
  hue="var(--d-backend)"
  perRow={3}
  nodes={[
    { id: "g", label: "G: goroutine", sub: "~2 KB stack" },
    { id: "p", label: "P: processor", sub: "local runq, 256" },
    { id: "m", label: "M: OS thread", sub: "runs G, needs P" },
    { id: "global", label: "Global runq", sub: "overflow" },
    { id: "steal", label: "Work stealing", sub: "idle P takes half" },
    { id: "syscall", label: "Syscall", sub: "P handed to new M" },
  ]}
  edges={[
    { from: "g", to: "p", label: "enqueue" },
    { from: "p", to: "m", label: "schedule" },
    { from: "p", to: "global", label: "spill" },
    { from: "steal", to: "p", label: "rebalance" },
    { from: "m", to: "syscall", label: "blocks" },
  ]}
  caption="GOMAXPROCS fixes the number of Ps — true parallelism. Ms float: a syscall pins an M in the kernel, so the runtime detaches its P and lets another thread drive the queue. Channel waits park the G without costing a thread at all."
/>
