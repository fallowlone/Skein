...
import costGap from "./infographics/cost-gap.svg?raw";
...
<Infographic
  svg={costGap}
  label="Side-by-side comparison of a goroutine versus an OS thread: the goroutine starts at ~2 KB of stack, switches in tens of nanoseconds in user space, fits 100k in ~200 MB, and is invisible to the OS; the OS thread reserves 8 MB up front, switches in 1-2 microseconds through the kernel, and 10,000 of them is an incident."
  caption="Stack size and switch cost differ by roughly three orders of magnitude — that gap is the entire reason 10,000 goroutines is routine while 10,000 threads is an incident."
  hue="var(--d-infra)"
/>
