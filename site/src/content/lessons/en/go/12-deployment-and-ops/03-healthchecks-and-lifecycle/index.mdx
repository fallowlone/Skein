...
import restartVsRoute from "./infographics/restart-vs-route.svg?raw";
...
<Infographic
  svg={restartVsRoute}
  label="Two-column comparison contrasting the liveness probe (failure restarts the container, threshold 3 x 10s = 30s, never check dependencies) against the readiness probe (failure removes the pod from Service endpoints, threshold 2 x 5s = ~10s, where hard-dependency checks belong)."
  caption="Same HTTP probe shape, opposite remedy: liveness kills and restarts, readiness reversibly pulls you from rotation — which is why dependency checks live only in readiness."
  hue="var(--d-infra)"
/>
