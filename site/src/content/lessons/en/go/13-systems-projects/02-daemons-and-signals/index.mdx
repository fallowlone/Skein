...
Signals are the supervisor's vocabulary, and each one is a promise you must keep:
...
And the mechanism that powered the Hook: `signal.Notify` does not *observe* a signal, it **claims** it — the runtime disables the default disposition for every signal you register. A registered-but-ignored SIGTERM produces a process that cannot be stopped politely at all.
