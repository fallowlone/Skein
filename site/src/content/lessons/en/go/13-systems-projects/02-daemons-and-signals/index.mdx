...
import signalContract from "./infographics/signal-contract.svg?raw";
...
<Infographic
  svg={signalContract}
  label="A four-item list of the supervisor signal vocabulary: SIGTERM is a graceful stop (stop accepting, drain in-flight, exit 0 inside the deadline); SIGINT is the same stop interactively via Ctrl-C; SIGHUP reloads config by re-reading, validating, and atomically swapping; SIGKILL and SIGSTOP are uncatchable and not yours."
  caption="The full lifecycle vocabulary: two signals stop you (SIGTERM/SIGINT), one reloads you (SIGHUP), and two are uncatchable (SIGKILL/SIGSTOP) — the existence of which is why the SIGTERM drain runs against a deadline."
  hue="var(--d-infra)"
/>
