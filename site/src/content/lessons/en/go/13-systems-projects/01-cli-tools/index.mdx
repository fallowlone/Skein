...
A serious tool has two audiences and one binary, so it needs an explicit machine format — `--json` — and a way to notice which audience is present. The mechanism is a TTY check on the *output* descriptor: `term.IsTerminal(int(os.Stdout.Fd()))` (from `golang.org/x/term`). Interactive terminal: render tables, color, progress spinners. Piped or redirected: drop all of it.
