...
import receiverTradeoff from "./infographics/receiver-tradeoff.svg?raw";
...
<Infographic
  svg={receiverTradeoff}
  label="Side-by-side comparison of Go's two method receiver kinds: a value receiver operates on a copy, lives on the stack with zero GC, is free of aliasing bugs, and suits small read-only methods; a pointer receiver operates on the original, avoids copying big payloads, is required for sync.Mutex, but risks a heap escape and data races."
  caption="The whole receiver choice in one frame: copy for small read-only data, pointer for mutation, big structs, or anything holding a sync.Mutex — never mix the two on one type."
  hue="var(--d-infra)"
/>
