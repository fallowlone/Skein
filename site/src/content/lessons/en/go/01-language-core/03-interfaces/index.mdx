...
<Crux>An interface value is a (type, value) pair, satisfied implicitly. A nil *T inside is NOT nil. Keep interfaces small.</Crux>
...
<Infographic
  svg={indirectionCost}
  label="Column chart of per-call cost in nanoseconds: a direct call is about 2 ns, an interface call about 4 ns, but a call that loses inlining and forces a heap escape jumps to about 50 ns."
  caption="The dispatch itself is near-free (~2 vs ~4 ns); the real bill is second-order — losing inlining plus boxing the value onto the heap turns a free access into ~50 ns, the 10-30x hot-loop hit."
  hue="var(--d-infra)"
/>
