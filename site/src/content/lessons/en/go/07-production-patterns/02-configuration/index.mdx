...
Config structs get printed: boot logs, crash reports, `%+v` in an error path, debug endpoints. Every one of those is a leak vector for the database password embedded in a DSN. Flags are worse — process arguments are world-readable in `ps` output on shared hosts. The Go-native containment is a named type whose formatting lies:
