...
<Infographic
  svg={bootVsRequest}
  label="Side-by-side comparison of the same malformed config value caught two ways: boot-time Load-and-validate fails inside the deploy where a readiness probe halts the rollout in about a minute with old pods still serving, while a lazy Getenv in the request path detonates at 3am days later, piles up goroutines, and OOMs the process."
  caption="Same bad value, two outcomes — where you read config decides who absorbs the failure: the rollout, or your users at 3am."
  hue="var(--d-infra)"
/>
