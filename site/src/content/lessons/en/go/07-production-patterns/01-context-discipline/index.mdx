...
import effectiveDeadline from "./infographics/effective-deadline.svg?raw";
...
<Infographic
  svg={effectiveDeadline}
  label="Column chart in seconds: parent budget 5, child requested 10, provider needs 7, effective deadline 5 — the effective deadline equals the smaller parent budget, not the child's larger request."
  caption="A derived context shrinks the deadline, never extends it. The effective deadline is the minimum along the path to the root (5s), so the call dies before the 10s the child asked for — and before the 7s the provider needed to answer."
  hue="var(--d-infra)"
/>
