...
<Infographic
  ...
  label="Column chart of offered load on a downed downstream as a multiple of the 1k-rps baseline: 1x with no retry, 3x with three attempts at one layer, 9x when two layers each retry, and 1.2x once a 10% retry budget caps each layer."
  caption="Layered unbudgeted retries turn a 1x baseline into 9x at the worst moment; a 10% budget brings it back to ~1.2x."
  hue="var(--d-infra)"
/>
