...
import levelContract from "./infographics/level-contract.svg?raw";
...
<Infographic
  svg={levelContract}
  label="A four-tier pyramid of slog levels ordered by who must act: Error at the apex pages a human, Warn covers degraded-but-self-healed events reviewed in aggregate, Info marks sparse state changes worth grepping, and Debug sits at the wide base — off in production and highest-volume."
  caption="Severity rises toward the apex while line volume falls: keep Error rare and high-signal, or the pager habituates and the one Error that matters drowns in the noise."
  hue="var(--d-infra)"
/>
