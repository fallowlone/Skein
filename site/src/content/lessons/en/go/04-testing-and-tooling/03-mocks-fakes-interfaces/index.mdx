...
import mockVsFake from "./infographics/mock-vs-fake.svg?raw";
...
<Infographic
  svg={mockVsFake}
  label="Side-by-side comparison of a mock versus a fake: the mock pins method, arguments, order, and count, so it shatters when two independent reads are reordered (214 tests red), is too obedient to catch real bugs, and is right only when the call itself is the behavior; the fake is a ~20-line map behind a mutex that survives behavior-preserving refactors, honors the real contract, and injects failure with one field."
  caption="A mock asserts interactions and so encodes the implementation; a fake asserts state and survives equivalent refactors. Reach for a mock only when the interaction itself is the behavior."
  hue="var(--d-infra)"
/>
