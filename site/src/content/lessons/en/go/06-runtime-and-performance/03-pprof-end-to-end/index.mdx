...
import inuseVsAlloc from "./infographics/inuse-vs-alloc.svg?raw";
...
<Infographic
  svg={inuseVsAlloc}
  label="Side-by-side comparison of the heap profile's two views: inuse_space (bytes currently live, attributed to the allocation site — the leak metric you diff between two snapshots) versus alloc_space (cumulative bytes ever allocated — the GC-pressure metric where churn hides, reaching 4 TB while retaining nothing)."
  caption="The single most-tested heap confusion: inuse_space answers 'what is retained?' (leaks); alloc_space answers 'what generates GC pressure?' (churn). A site can lead alloc_space with zero inuse."
  hue="var(--d-infra)"
/>
