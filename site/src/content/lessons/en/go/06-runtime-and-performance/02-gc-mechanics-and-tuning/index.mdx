...
<FlowDiagram
  label="One GC cycle: a sub-ms pause turns on write barriers, concurrent marking drains grey objects while mutators run, a second pause confirms termination, sweep is amortized into allocation"
  hue="var(--d-backend)"
  perRow={3}
  nodes={[
    { id: "trigger", label: "Pacer triggers", sub: "heap ≈ live×(1+GOGC/100)" },
    { id: "stw1", label: "STW #1", sub: "enable write barrier" },
    { id: "mark", label: "Concurrent mark", sub: "25% CPU + assists" },
    { id: "barrier", label: "Write barrier", sub: "shades dirty ptrs" },
    { id: "stw2", label: "STW #2", sub: "mark termination" },
    { id: "sweep", label: "Lazy sweep", sub: "freed on alloc" },
  ]}
  edges={[
    { from: "trigger", to: "stw1", label: "start cycle" },
    { from: "stw1", to: "mark", label: "µs pause" },
    { from: "barrier", to: "mark", label: "feeds grey set" },
    { from: "mark", to: "stw2", label: "grey drained" },
    { from: "stw2", to: "sweep", label: "µs pause" },
  ]}
  caption="GOMEMLIMIT bends the trigger: as runtime memory nears the limit, cycles start earlier and run harder — down to back-to-back marking capped at 50% CPU. Headroom between live heap and the limit is what keeps this regime an emergency brake instead of a steady state."
/>
