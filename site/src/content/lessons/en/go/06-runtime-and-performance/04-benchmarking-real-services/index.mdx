...
<Infographic
  ...
  label="Side-by-side comparison of a closed-loop and an open-loop load generator running the same service at 5,000 rps: the closed-loop runner uses a fixed pool of virtual users that stop arriving during stalls, so its histogram omits the worst seconds and reports p99 = 31 ms; the open-loop runner fires at a fixed arrival rate so stall time enters the histogram, reporting p99 = 2.1 s — 10 to 100 times higher."
  caption="Same service, same load: the closed loop coordinates with stalls and hides the tail (31 ms), the open loop measures it honestly (2.1 s). The loop model — not the data — is the variable that moves p99 by 10–100×."
  hue="var(--d-infra)"
/>
