...
<Infographic
  ...
  label="Column chart: with a healthy 300 ms p99, a 20-connection pool sustains about 67 transactions per second; when downstream p99 degrades to 9 seconds inside the transaction, the ceiling collapses to about 2.2 per second."
  caption="A Tx pins one connection for its whole lifespan, so one slow HTTP call inside it divides the pool's throughput by the latency — a roughly 30x collapse while the database sits at 2% CPU."
  hue="var(--d-infra)"
/>
