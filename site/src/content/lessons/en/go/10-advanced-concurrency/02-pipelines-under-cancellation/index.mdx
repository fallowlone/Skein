...
<Infographic
  ...
  label="Column chart of throughput in thousands of records per second: the producer stage runs at 12k/s, but the consumer stage, the buffered pipeline, and the unbuffered pipeline all sit at 9k/s."
  caption="A 1024-slot buffer changes nothing in steady state: buffered and unbuffered both settle at the consumer's 9k/s. The slowest stage sets throughput; a buffer only absorbs jitter, never a persistent rate gap."
  hue="var(--d-infra)"
/>
