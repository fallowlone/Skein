...
<Infographic
  svg={timeoutBudget}
  label="Horizontal bar chart of the four http.Server timeout fields in seconds: ReadHeaderTimeout 5, ReadTimeout 10, WriteTimeout 30, IdleTimeout 120 — the budget climbs from tightest at the header phase to most generous for idle keep-alive."
  caption="These four fields all default to zero (infinite). Set the header window tight to kill slowloris, then widen the budget through body, response, and idle phases."
  hue="var(--d-infra)"
/>
