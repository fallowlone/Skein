...
- encapsulation
status: ready
summary: How an unmitigated stampede pushes a system into a self-sustaining retry
  storm it cannot escape, the fencing token fix for lock TTL races, four production
  stampede postmortems, and a three-tier global cache design exercise.
...
Together these five steps explain why a 10-second stampede can hold your DB at 100% CPU for hours: the retry amplification alone keeps the DB too busy to complete even a single cache rebuild, and without the cache the arrivals keep retrying. Without an external kill signal, no component in the loop can break it.
...
<Infographic
  svg={herdFunnel}
  label="A funnel showing the herd shrinking by an order of magnitude at each cache tier: 10,000 concurrent misses, the CDN's SWR and request coalescing absorb 99% leaving about 100 to reach the origin, XFetch and single-flight at the application cache collapse per-node herds to about 10, and the Redis lock collapses the cross-node herd to a single rebuild."
  caption="Defense in depth: each tier independently drops the herd by ~10×, so 10,000 simultaneous misses collapse to a single rebuild — no one layer has to be perfect."
  hue="var(--d-infra)"
/>
...
<FlowDiagram
  label="A stampede saturates the database, which makes queries time out, which makes clients retry; the retries add load that keeps the database saturated and blocks the cache rebuild, so the cache stays empty and feeds still more retries — a self-sustaining loop only an external kill signal can break."
  hue="var(--d-backend)"
  perRow={4}
  nodes={[
    { id: "stampede", label: "Stampede", sub: "herd hits DB" },
    { id: "db", label: "DB saturated", sub: "100% CPU" },
    { id: "timeout", label: "Timeouts", sub: "queue past limit" },
    { id: "retry", label: "Client retries", sub: "amplify herd" },
  ]}
  edges={[
    { from: "stampede", to: "db" },
    { from: "db", to: "timeout" },
    { from: "timeout", to: "retry" },
    { from: "retry", to: "db", label: "loop" },
  ]}
  caption="Each component follows correct local behaviour, yet the retries → DB-saturation → timeouts → more-retries cycle is self-sustaining. Only an external kill (503 gate, circuit-breaker, manual warmup) breaks it."
/>
