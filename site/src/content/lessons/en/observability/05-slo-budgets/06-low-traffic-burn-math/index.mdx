...
summary: "Low-traffic services break naive SLO arithmetic — solutions include synthetic probes, aggregation, and longer windows. The six SLO alerting approaches from the Google SRE Workbook show exactly why five fail. Every canonical burn-rate threshold is derivable from one equation."
estMin: 17
status: ready
lessonType: topic
level: senior
prereqs: ["observability/05-slo-budgets/05-slo-platforms-rollout"]
deepensInto: []
spiral: []
concepts: ["low-traffic-slo", "slo-alerting-approaches", "burn-rate-derivation", "synthetic-probes"]
sources:
