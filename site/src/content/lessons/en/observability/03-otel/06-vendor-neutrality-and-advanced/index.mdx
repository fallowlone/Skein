...
<Infographic
  svg={erosionPaths}
  label="Side-by-side comparison of the two erosion paths of OTel vendor-neutrality: a vendor SDK living in application code versus a proprietary processor living in the Collector config — each shown with where it lives, its migration cost, and how to audit for it."
  caption="Emitting OTLP is necessary but not sufficient. Lock-in hides either in app code (a vendor SDK) or in the Collector config (a proprietary processor) — the quarterly portability audit checks both layers."
  hue="var(--d-infra)"
/>
