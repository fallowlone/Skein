...
Before you add an auto-retry to your HTTP client library, ask yourself: is this method idempotent? The answer determines whether retrying saves your users or creates duplicate orders. HTTP defines verbs (methods) for what the client wants: `GET` (read), `POST` (create or generic non-idempotent), `PUT` (idempotent replace), `DELETE` (idempotent remove), `PATCH` (partial update), `HEAD` (metadata-only GET), `OPTIONS` (capabilities check), `CONNECT` (tunnel for proxies).
...
<Infographic
  ...
  label="Three-step spectrum of the SameSite cookie attribute from Strict to Lax to None, each step sending the cookie on more cross-site requests in exchange for weaker CSRF protection."
  caption="SameSite is a CSRF-vs-reach dial: Strict never sends cross-site, Lax (the default) allows safe navigations, None sends everywhere but gives no CSRF protection."
  hue="var(--d-infra)"
/>
...
When you deploy a new service, these four headers should be on your first-day checklist — each one closes a class of attack that you cannot close at the application layer. Common response headers that harden the browser's behaviour:
...
<SequenceDiagram
  label="CORS preflight exchange"
  hue="var(--d-network)"
  actors={["Browser", "Server"]}
  messages={[
    { from: "Browser", to: "Server", label: "OPTIONS (preflight)" },
    { from: "Server", to: "Browser", label: "200 + Access-Control-Allow-*" },
    { from: "Browser", to: "Server", label: "PUT /resource (actual request)" },
    { from: "Server", to: "Browser", label: "200 + Access-Control-Allow-Origin" },
  ]}
  caption="For a non-simple method the browser sends an OPTIONS preflight first; only after the server allows it does the actual request proceed."
/>
