...
import ttlCacheHitRate from "./infographics/ttl-cache-hit-rate.svg?raw";
...
<Infographic
  svg={ttlCacheHitRate}
  label="Column chart of cache hit rate against TTL at a steady 10 queries per hour: TTL 60 s reaches ~17% hits, TTL 300 s ~50%, TTL 3600 s ~90%, and TTL 86400 s ~99%."
  caption="Cache hits climb steeply at first, then flatten — past 3600 s a longer TTL barely adds hit rate while multiplying how long stale data lingers."
  hue="var(--d-infra)"
/>
