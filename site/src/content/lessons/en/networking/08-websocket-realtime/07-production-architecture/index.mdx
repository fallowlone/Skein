...
- latency
status: ready
summary: How to configure Nginx and ALB for long-lived upgrades, harden against DoS
  and cache-poisoning, and scale WebSocket clusters with pub/sub and sticky sessions.
...
<Infographic
  svg={timeoutBudget}
  label="Column chart of WebSocket timeout values in seconds: proxy default idle timeout 60, server ping interval 30, slow-reader close threshold 30, production read/send timeout 3600."
  caption="The 60-second default idle timeout is the cliff: raise read/send to 3600s and ping every 25–30s so traffic always arrives under the limit."
  hue="var(--d-infra)"
/>
...
<FlowDiagram
  label="Distributed WebSocket fan-out: User A connects through a sticky load balancer to WebSocket Server 1, which publishes the message to a Redis stream; Server 2 is subscribed to the same stream and broadcasts it to its locally connected User B"
  hue="var(--d-network)"
  perRow={3}
  nodes={[
    { id: "ua", label: "User A", sub: "sends message" },
    { id: "lb", label: "Load balancer", sub: "sticky session" },
    { id: "s1", label: "WS Server 1", sub: "A connected here" },
    { id: "redis", label: "Redis stream", sub: "pub/sub, msgId" },
    { id: "s2", label: "WS Server 2", sub: "subscribed" },
    { id: "ub", label: "User B", sub: "different server" },
  ]}
  edges={[
    { from: "ua", to: "lb", label: "upgrade" },
    { from: "lb", to: "s1", label: "route (sticky)" },
    { from: "s1", to: "redis", label: "publish" },
    { from: "redis", to: "s2", label: "subscribe" },
    { from: "s2", to: "ub", label: "broadcast" },
  ]}
  caption="Pub/sub decouples senders from receivers: a message from any server reaches clients on any other server, while sticky sessions keep each client's in-flight state on one server across reconnects."
/>
