...
summary: "Sticky sessions pin clients to one backend and cause 1.5–2.5x load imbalance and session loss on failure; consistent hashing maps keys to backends with minimal remapping on membership changes; the right fix is externalizing session state to Redis."
estMin: 12
status: ready
lessonType: topic
level: middle
