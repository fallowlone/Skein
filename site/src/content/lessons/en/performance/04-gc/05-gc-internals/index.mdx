...
<Infographic
  svg={gcTradeoff}
  label="Two-column comparison of G1 versus ZGC on a 16 GB heap across three metrics: G1 has ~60 ms pauses with baseline throughput and memory; ZGC has sub-millisecond pauses but throughput drops 12% and memory grows 18%."
  caption="The GC three-way tradeoff: ZGC buys sub-millisecond pauses by spending throughput and memory. No collector wins all three at once — these numbers are the expected cost, not a bug."
  hue="var(--d-infra)"
/>
