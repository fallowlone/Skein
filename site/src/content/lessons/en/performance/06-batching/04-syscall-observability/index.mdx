...
import syscallCollapse from "./infographics/syscall-collapse.svg?raw";
...
<Infographic
  svg={syscallCollapse}
  label="A funnel narrowing across three batching approaches: syscalls per one million operations collapse from 1,000,000 with one write() per op, to about 1,000 with a buffered writev() or io_uring batch, to about zero with io_uring plus SQPOLL under load."
  caption="Each batching step removes more user↔kernel crossings: one write() per op burns 1,000,000 syscalls per 1M ops, a buffered writev()/io_uring_enter() batch cuts it to ~1,000, and SQPOLL polling drives it to ~0 under sustained load."
  hue="var(--d-infra)"
/>
...
<FlowDiagram
  label="Each syscall pays a fixed user↔kernel crossing; batching amortizes it over N ops"
  hue="var(--d-hardware)"
  nodes={[
    { id: "wn", label: "N writes", sub: "1 syscall each", col: 0, row: 0 },
    { id: "crossn", label: "N crossings", sub: "user↔kernel ×N", col: 1, row: 0 },
    { id: "slow", label: "N × 1–5µs", sub: "+ cache thrash", col: 2, row: 0 },
    { id: "buf", label: "buffer N", sub: "in userspace", col: 0, row: 1 },
    { id: "one", label: "1 crossing", sub: "writev / io_uring_enter", col: 1, row: 1 },
    { id: "fast", label: "~1–5µs total", sub: "SQPOLL → ~0", col: 2, row: 1 },
  ]}
  edges={[
    { from: "wn", to: "crossn" },
    { from: "crossn", to: "slow" },
  ...
  caption="Per-op crossings pay the ~1–5µs trap N times; one batched writev (or io_uring_enter) carries all N records across the boundary once. SQPOLL polls the ring for near-zero crossings under load."
/>
