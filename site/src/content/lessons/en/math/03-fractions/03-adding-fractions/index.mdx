...
status: ready
summary: Add fractions with the same bottom number by adding the tops — and when the
  bottoms differ, make them match first with equivalent fractions.
...
together. When you see two fractions with different bottoms, your first move is always
the same: find a number that both denominators divide into. First you must rewrite both
fractions so they share one denominator. A denominator that both can be rewritten to is
...
denominator is the size of a part, not a count. Now when you see two fractions to add,
check the denominators first: if they match, go straight to the numerators; if they
differ, find a common denominator before touching anything else.
</Recap>
