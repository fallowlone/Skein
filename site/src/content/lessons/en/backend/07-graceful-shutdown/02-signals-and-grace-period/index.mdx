...
summary: "Shutdown is a contract with the orchestrator: a preStop hook, then SIGTERM to PID 1, then a grace period (Kubernetes default 30s), then an unstoppable SIGKILL. The grace period is a hard deadline, and the classic bug is a process that never receives the signal at all."
estMin: 15
status: ready
lessonType: topic
level: middle
...
concepts: ["sigterm", "sigkill", "termination-grace-period", "prestop-hook", "pid-1", "signal-forwarding", "hard-deadline"]
sources:
