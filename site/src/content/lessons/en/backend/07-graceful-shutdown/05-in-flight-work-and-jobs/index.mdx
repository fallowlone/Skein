...
summary: "The grace period is a budget, and long requests or background jobs may not fit. The senior question is what to do with work that won't finish: reject it cleanly, or requeue it — and requeue is only safe when the consumer is idempotent, tying back to the idempotency unit."
estMin: 17
status: ready
lessonType: topic
level: senior
...
spiral: ["failure-modes", "resilience", "lifecycle", "resource-limits", "idempotency"]
concepts: ["deadline-budget", "long-running-request", "background-job", "requeue", "visibility-timeout", "checkpointing", "at-least-once"]
sources:
