...
<Infographic
  svg={twoClocks}
  label="Side-by-side comparison of the two paths a terminating pod starts at once. The signal path is the fast clock: the kubelet sends SIGTERM in one local hop, delivered in milliseconds, and the listener closes immediately. The routing path is the slow clock: the API server marks the endpoint not-ready, it propagates to every node's kube-proxy and lags behind external load balancers or polling ingress controllers, draining in under a second on small clusters but 10 to 30 seconds on large ones."
  caption="The signal clock finishes in milliseconds while the routing clock runs up to 10–30s — that gap is the race window where a closed listener refuses traffic the LB is still sending."
  hue="var(--d-backend)"
/>
