...
import drainWindow from "./infographics/drain-window.svg?raw";
...
<Infographic
  svg={drainWindow}
  label="Column chart of the deregistration drain window in seconds: default low 30, default high 60, and long-lived connections or uploads 600 — a roughly tenfold jump."
  caption="The drain window is not one small fixed number: cloud LB defaults run 30–60s, but long-lived connections or large uploads push it to 600s+, sized to ~3× p99 and clamped by the guardian timeout."
  hue="var(--d-infra)"
/>
