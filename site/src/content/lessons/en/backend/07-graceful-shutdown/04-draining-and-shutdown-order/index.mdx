...
import orderVsDeadline from "./infographics/order-vs-deadline.svg?raw";
...
<Infographic
  svg={orderVsDeadline}
  label="A two-by-two of shutdown outcomes: rows are closing order (correct reverse-dependency versus concurrent), columns are whether the drain finishes before the grace period. Only correct order with a completing drain exits cleanly; the guardian timeout rescues only the timing axis; wrong order loses in-flight requests to a dead pool whether the drain finishes or hangs."
  caption="Order and the guardian timeout are orthogonal: the timeout only turns a hang into a clean self-exit — it cannot save requests a mis-ordered teardown already severed. You need both."
  hue="var(--d-infra)"
/>
