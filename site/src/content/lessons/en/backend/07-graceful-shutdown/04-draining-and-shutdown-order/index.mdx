...
summary: "Draining is more than server.close(): keep-alive sockets must be closed deliberately, and resources must close in reverse dependency order — HTTP server first, datastores last — or in-flight requests hit a closed pool. A guardian timeout force-exits if the drain hangs."
estMin: 16
status: ready
lessonType: topic
level: middle
...
spiral: ["failure-modes", "resilience", "lifecycle", "resource-limits"]
concepts: ["connection-draining", "keep-alive", "reverse-dependency-order", "guardian-timeout", "graceful-server-close", "resource-teardown"]
sources:
