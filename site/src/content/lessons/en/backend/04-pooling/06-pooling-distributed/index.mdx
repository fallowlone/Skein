...
- scalability
status: ready
summary: One pool against one database is solved. The hard version is N application
  instances each with their own pool, all colliding against a single max_connections
  — where the math overflows and a transaction-level multiplexer becomes mandatory,
...
<Infographic
  svg={poolingModes}
  label="Side-by-side comparison of PgBouncer session pooling versus transaction pooling. Session pooling holds a real backend for a whole client session, multiplexes at about 1x, and keeps session state safe. Transaction pooling holds a backend for one transaction's milliseconds, reaching a 25 to 50x ratio that collapses 1000 client connections to about 25 real backends, but breaks server-side prepared statements, SET, LISTEN/NOTIFY, and advisory locks."
  caption="Transaction pooling buys the huge ratio precisely by not letting any client keep a backend — the same mechanism that breaks session state across transactions. Session pooling preserves that state but barely reduces the connection count."
  hue="var(--d-infra)"
/>
...
<FlowDiagram
  label="PgBouncer fan-in: many app pools into few real Postgres backends"
  hue="var(--d-backend)"
  perRow={4}
  nodes={[
    {id:"app1",label:"app inst 1",sub:"pool size 20"},
    {id:"app2",label:"app inst 2",sub:"pool size 20"},
    {id:"appN",label:"app inst N",sub:"pool size 20"},
    {id:"pgb",label:"PgBouncer",sub:"lightweight clients"},
    {id:"backends",label:"~25 backends",sub:"real connections"},
    {id:"pg",label:"Postgres",sub:"max_connections=100"}
  ]}
  edges={[
    {from:"app1",to:"pgb",label:"1000 total"},
    {from:"app2",to:"pgb"},
    {from:"appN",to:"pgb"},
    {from:"pgb",to:"backends",label:"txn pooling"},
    {from:"backends",to:"pg",label:"25–50× ratio"}
  ]}
  caption="50 instances × pool 20 = 1,000 client connections fan into PgBouncer. With transaction pooling PgBouncer holds only ~25 real Postgres backends — a 40× multiplexing ratio — keeping the fleet well under max_connections = 100."
/>
