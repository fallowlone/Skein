...
summary: "When every connection is busy, a request does not fail — it waits in a queue. The acquisition timeout decides how long it waits before giving up, and that one number is the difference between failing fast and a pile-up that takes the whole service down."
estMin: 15
status: ready
lessonType: topic
level: middle
...
concepts: ["connection-acquisition", "wait-queue", "acquire-timeout", "fail-fast", "queue-buildup", "latency-budget", "thread-starvation"]
sources:
