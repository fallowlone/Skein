...
import failFastVsPileUp from "./infographics/fail-fast-vs-pile-up.svg?raw";
...
<Infographic
  svg={failFastVsPileUp}
  label="Side-by-side comparison of two acquisition-timeout choices on a starved pool: a tuned 250 ms–2 s timeout frees the upstream thread, rides normal micro-bursts, and preserves capacity, while the 30 s default pins each waiter's thread for 30 s, fills the thread pool with waiters, and turns a slow database into a full outage."
  caption="The timeout is one dial with two opposite outcomes: bound the wait to fail fast and preserve capacity, or leave the 30 s default and let a slow database starve a healthy web tier."
  hue="var(--d-backend)"
/>
