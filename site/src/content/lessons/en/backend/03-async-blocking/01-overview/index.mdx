...
summary: "A server spends most of its life waiting on I/O. Blocking I/O parks a whole thread on each wait, so concurrency costs memory; non-blocking I/O hands the wait to the kernel and lets one thread juggle thousands of sockets through an event loop."
estMin: 11
status: ready
lessonType: topic
level: junior
...
spiral: ["latency"]
concepts: ["blocking-io", "non-blocking-io", "thread-per-connection", "event-loop", "epoll", "c10k", "concurrency-vs-parallelism"]
sources:
