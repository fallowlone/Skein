...
import twoEquilibria from "./infographics/two-equilibria.svg?raw";
...
<Infographic
  svg={twoEquilibria}
  label="A two-column comparison of the same system's two stable equilibria. The healthy state: short queue that drains fast, fast ~40ms responses, few retries, holds with no intervention. The collapsed state: full self-refilling queue, slow 4s-timeout responses driven by a retry storm, and it survives even after the provider recovers because fixing the trigger does not free the feedback loop."
  caption="Metastability means the system has two stable states — a big enough shock flips it from healthy to collapsed, and it stays collapsed even after the original cause is gone."
  hue="var(--d-infra)"
/>
