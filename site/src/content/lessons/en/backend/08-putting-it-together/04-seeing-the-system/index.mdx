...
- resource-limits
status: ready
summary: 'A backend you can''''''''t observe is one you can''''''''t operate. This
  lesson ties the mechanisms to telemetry: rate, errors, and duration (RED), the p99
  tail behind the mean, pool saturation and queue depth, breaker transitions, and
  the error budget that gates shipping against reliabi'
...
import meanVsP99 from "./infographics/mean-vs-p99.svg?raw";
...
<Infographic
  svg={meanVsP99}
  label="Column chart of one latency distribution — 99 requests at 20ms plus one at 4 seconds — summarized three ways: p50 is 20ms, the mean is 60ms, and p99 is 4000ms, so the mean bar stays tiny while the p99 bar towers."
  caption="One distribution, three summaries: p50 20ms and mean 60ms look calm, but p99 is 4000ms — the average drowns the slow tail every user actually feels."
  hue="var(--d-infra)"
/>
...
<StackDiagram
  label="Observability layers — outermost to innermost"
  hue="var(--d-backend)"
  layers={[
    {label:"SLO + error budget", note:"ship/stop decision"},
    {label:"RED metrics", note:"rate · errors · duration"},
    {label:"Percentile latency", note:"p99 / p999, not mean"},
    {label:"Pool saturation", note:"in-use / limit + acquire wait"},
    {label:"Breaker state", note:"closed → open → half-open"},
    {label:"Loop lag + retry rate", note:"loop block · storm signal"},
  ]}
  caption="The cascade shows first in p99, then pool saturation. Mean latency stays calm until collapse — never trust averages."
/>
