...
summary: "Every mechanism is also a load-control knob, and overload is where they combine: pool and concurrency caps bound work in flight, timeouts and breakers cut losses, and load shedding rejects excess early with backpressure — so the service degrades gracefully instead of collapsing."
estMin: 18
status: ready
lessonType: topic
level: senior
...
spiral: ["synthesis", "resource-limits", "resilience", "failure-modes"]
concepts: ["load-shedding", "graceful-degradation", "backpressure", "concurrency-limit", "goodput"]
sources:
