...
summary: "A circuit breaker is a three-state machine: closed passes calls and counts failures, open rejects every call instantly for a cooldown, half-open lets a few trial calls test recovery. The cooldown timer is the recovery dial; half-open stops a recovering service being flooded."
estMin: 14
status: ready
lessonType: topic
level: middle
...
concepts: ["circuit-breaker", "closed-state", "open-state", "half-open-state", "reset-timeout", "trial-call", "thundering-herd"]
sources:
