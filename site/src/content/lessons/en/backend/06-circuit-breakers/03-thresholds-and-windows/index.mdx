...
summary: "A breaker should not trip on a single failure or stay closed through a meltdown. What trips it is a failure rate over a sliding window, gated by a minimum-volume floor so it ignores noise — and a slow call counted as a failure, because slow is the state that hurts."
estMin: 15
status: ready
lessonType: topic
level: middle
...
concepts: ["failure-rate-threshold", "sliding-window", "count-based-window", "time-based-window", "minimum-volume", "slow-call-rate", "false-trip"]
sources:
