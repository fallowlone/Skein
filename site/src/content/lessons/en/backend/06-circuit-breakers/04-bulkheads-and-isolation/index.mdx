...
summary: "A breaker reacts after failures accumulate, but a shared thread pool can be drained by one slow dependency before it trips. A bulkhead caps concurrency per dependency so one sick downstream can't take the whole budget — thread-pool vs semaphore isolation is a real tradeoff."
estMin: 15
status: ready
lessonType: topic
level: middle
...
spiral: ["failure-modes", "resilience", "resource-limits"]
concepts: ["bulkhead", "failure-isolation", "thread-pool-isolation", "semaphore-isolation", "resource-partition", "concurrency-limit", "blast-radius"]
sources:
