...
import isolationTradeoff from "./infographics/isolation-tradeoff.svg?raw";
...
<Infographic
  svg={isolationTradeoff}
  label="Side-by-side comparison of thread-pool isolation versus semaphore isolation: thread-pool runs each call on a separate pool thread so the caller can abandon a hung call via its own timeout at the cost of a hand-off per call (Hystrix coreSize 10); semaphore runs on the caller's own thread at near-zero overhead but can't unstick a hang and only stops new calls (resilience4j maxConcurrentCalls 25)."
  caption="Both cap concurrency per dependency — but only a separate thread lets the caller walk away from a hung call, which is why thread pools guard network I/O and semaphores guard fast non-blocking calls."
  hue="var(--d-infra)"
/>
