...
summary: "Everything so far assumed one process. Across many instances a breaker becomes a per-instance guess with no shared view, retries multiply into call amplification, half-open probes herd, and the only stable shedding is coordinated. Resilience as a fleet property."
estMin: 18
status: ready
lessonType: topic
level: senior
...
spiral: ["failure-modes", "resilience", "distributed-systems"]
concepts: ["per-instance-state", "retry-amplification", "retry-storm", "thundering-herd", "jittered-probe", "exponential-backoff", "token-bucket-retry", "coordinated-load-shedding"]
sources:
