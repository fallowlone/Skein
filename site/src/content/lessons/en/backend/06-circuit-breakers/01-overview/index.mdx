...
<Infographic
  svg={threadHoldTime}
  label="Column chart of how long a worker thread stays pinned per call: a healthy call holds it 50 ms, a slow dependency without a breaker holds it 5000 ms (the full timeout), and a fast-fail with a breaker frees it in about 1 ms."
  caption="A slow timeout pins a thread ~100× longer than a healthy call; the breaker's fast-fail returns it to near-zero (~1 ms), close to the healthy baseline."
  hue="var(--d-backend)"
/>
