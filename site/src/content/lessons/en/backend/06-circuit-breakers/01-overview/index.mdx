...
summary: "A dependency rarely fails cleanly — it gets slow, and a slow dependency is more dangerous than a dead one because every caller waits, occupying a thread until the whole service starves. A circuit breaker fast-fails calls to a sick dependency so the caller stops waiting."
estMin: 12
status: ready
lessonType: topic
level: junior
...
concepts: ["circuit-breaker", "cascading-failure", "fast-fail", "thread-starvation", "fault-isolation", "timeout"]
sources:
