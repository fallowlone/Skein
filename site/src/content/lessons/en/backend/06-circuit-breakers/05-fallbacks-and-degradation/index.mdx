...
summary: "A breaker only works if a timeout makes a hang look like a failure, and an open breaker only helps if the caller has an answer ready. The senior view: a timeout is the trigger, a fallback is the answer, graceful degradation should be rare, and load shedding is the last line."
estMin: 16
status: ready
lessonType: topic
level: senior
...
spiral: ["failure-modes", "resilience"]
concepts: ["timeout", "fallback", "graceful-degradation", "load-shedding", "default-response", "stale-cache", "fail-open-vs-fail-closed"]
sources:
