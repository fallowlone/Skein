...
import fallbackLadder from "./infographics/fallback-ladder.svg?raw";
...
<Infographic
  svg={fallbackLadder}
  label="A five-tier pyramid ranking circuit-breaker fallbacks from best at the top to last resort at the bottom: a degraded-but-correct response, then last-known-good cache, a static default, queue-for-later via the outbox, and a fail-fast 503 with Retry-After as the floor."
  caption="An open breaker must return something. Reach for the highest fallback the call site allows; a clean fail-fast 503 is the floor, never a raw hang."
  hue="var(--d-backend)"
/>
