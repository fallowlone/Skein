...
summary: "Direct dual-write to DB + Kafka breaks in either order. The outbox pattern writes the event inside the DB transaction; the inbox pattern deduplicates on the consumer side. Together they give effectively-once delivery."
estMin: 16
status: ready
lessonType: topic
level: middle
...
spiral: ["statefulness"]
concepts: ["dual-write", "outbox-pattern", "inbox-pattern", "CDC", "debezium", "dead-letter-queue", "effectively-once"]
sources:
