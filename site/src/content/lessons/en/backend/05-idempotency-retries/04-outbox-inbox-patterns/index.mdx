...
<Infographic
  svg={relayTradeoff}
  label="Side-by-side comparison of two outbox-relay implementations: a poll-based relay adds 1–5 seconds of latency equal to its poll interval, needs no extra infrastructure, but loads the database with repeated scans; a Debezium CDC relay tails the Postgres write-ahead log for sub-second lag, publishes the instant a row commits, with no polling overhead — the production standard for high-throughput services."
  caption="Both deliver at-least-once; CDC trades a WAL-tailing dependency for sub-second lag and zero polling load — the default once throughput is high."
  hue="var(--d-backend)"
/>
...
<FlowDiagram
  label="Outbox → relay → inbox: effectively-once across the dual-write boundary"
  hue="var(--d-backend)"
  perRow={4}
  nodes={[
    { id: "txn", label: "DB transaction", sub: "BEGIN" },
    { id: "order", label: "orders row", sub: "business write" },
    { id: "outbox", label: "outbox row", sub: "published=false" },
    { id: "commit", label: "COMMIT", sub: "atomic" },
    { id: "relay", label: "Relay", sub: "poll / CDC" },
    { id: "kafka", label: "Kafka", sub: "broker ACK" },
    { id: "inbox", label: "inbox dedup", sub: "event_id UNIQUE" },
    { id: "effect", label: "Business effect", sub: "once only" },
  ]}
  edges={[
    { from: "txn", to: "order" },
    { from: "txn", to: "outbox" },
    ...
    { from: "commit", to: "relay", label: "relay picks up" },
    { from: "relay", to: "kafka", label: "publish" },
    { from: "kafka", to: "inbox", label: "consume" },
    { from: "inbox", to: "effect", label: "first seen" },
  ]}
  caption="Crash before COMMIT: both rows vanish — no phantom event. Crash after COMMIT: relay retries; inbox dedup ensures the effect runs exactly once."
/>
