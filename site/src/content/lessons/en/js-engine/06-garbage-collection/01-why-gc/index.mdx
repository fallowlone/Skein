...
<AlgoPoster client:visible
  id="ap-reachability"
  lang="en"
  kicker="roots → live · island → garbage"
  lessonTag="lesson: js-engine / 06 / 01"
  cost="trace live, ignore the rest"
  title="REACHABLE IS LIVE — THE ISLAND IS GARBAGE"
  titleHi={["REACHABLE", "ISLAND"]}
  sub="Roots reach A, B, and C through live edges while X and Y point only at each other. The trace marks everything on a root path and reclaims the unreached island — cycles included."
  badges={["roots: stack · global · handles", "live: on a root path", "cycles: no rescue"]}
  panelTitle="REACHABILITY AS A WAVE · ~9s / cycle"
  label="Animated graph wave: roots light up A, B, and C while the X-Y island never lights — collectible despite its internal references."
  pattern="graph"
  data={{ nodes: ["roots", "A", "B", "C", "X", "Y"], edges: [[0, 1], [1, 2], [0, 3], [4, 5], [5, 4]], waves: [[0], [1, 3], [2], []], dones: [[0], [0, 1, 3], [0, 1, 2, 3], [0, 1, 2, 3]], dists: [[null, null, null, null, null, null], ["0", "1", null, "1", null, null], ["0", "1", "2", "1", null, null], ["0", "1", "2", "1", "—", "—"]], note: "X↔Y: refs intact, still garbage" }}
  steps={[
    { caption: "① roots: stack, globals, embedder handles…", state: "STATE: roots live", log: "roots · always live" },
    { caption: "② wave: A and C reached, then B…", state: "STATE: tracing", log: "roots → A · C → B" },
    { caption: "③ live: everything on a root path…", state: "STATE: live marked", log: "A · B · C live" },
    { caption: "④ island: X and Y unreachable — collect…", state: "STATE: island garbage ✓", log: "X↔Y · no path ✓" }
  ]}
  minis={[
    { title: "CYCLES — tracing wins", body: "X and Y reference each other, so refcounting would pin both forever. Tracing asks only whether a root reaches them — it does not.", tone: 0, at: 3 },
    { title: "OVER-APPROXIMATION — live, not needed", body: "Reachability keeps everything touchable, including logically dead caches. A leak is live-but-unwanted — lesson 05's retainer paths.", tone: 2, at: 2 }
  ]}
  takeaway="takeaway: never enumerate the dead — trace the live and take the rest."
  src="src: lesson js-engine/06 · why gc"
/>
