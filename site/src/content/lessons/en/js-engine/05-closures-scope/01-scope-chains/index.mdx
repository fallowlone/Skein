...
<AlgoPoster client:visible
  id="ap-scope-chain"
  lang="en"
  kicker="(depth, slot) · no name search"
  lessonTag="lesson: js-engine / 05 / 01"
  cost="read: depth hops + 1 load"
  title="NAMES RESOLVE AT COMPILE — SLOTS AT RUNTIME"
  titleHi={["COMPILE", "RUNTIME"]}
  sub="ScopeInfo resolves each variable to a fixed depth-and-slot pair at compile time. At runtime the read is just outer-pointer hops plus one indexed load — no hashing, no names."
  badges={["ScopeInfo: depth + slot", "Context: slots + outer", "locals: registers, free"]}
  panelTitle="THE RUNTIME SCOPE CHAIN · ~9s / cycle"
  label="Animated four-stage pipeline: inner Context points outward through outer scopes to script and global, and one read hops twice then loads."
  pattern="pipeline"
  data={{ title: "total at (depth 1, slot 0)", stages: ["inner()|slot: i", "outer()|slot: total", "script|let/const cells", "global|var + fns"], at: [0, 1, 2, 3], dones: [[1, 0, 0, 0], [1, 1, 0, 0], [1, 1, 1, 0], [1, 1, 1, 1]], note: "hop outer, then load slot" }}
  steps={[
    { caption: "① inner runs: i lives in its own Context…", state: "STATE: inner Context", log: "inner · slot i" },
    { caption: "② total is one scope out: follow outer…", state: "STATE: hop outer", log: "outer → total slot" },
    { caption: "③ script and global end the chain…", state: "STATE: chain ends", log: "script · global" },
    { caption: "④ the read: two hops, one indexed load…", state: "STATE: resolved ✓", log: "(1, 0) · 1 load ✓" }
  ]}
  minis={[
    { title: "COMPILE-TIME — names die early", body: "The parser bakes total into (depth 1, slot 0) inside the bytecode. At runtime no name exists to look up.", tone: 0, at: 0 },
    { title: "DYNAMIC — with and eval poison it", body: "A with block or sloppy eval hides names until runtime, so the compiler emits a slow named lookup and the scope resists optimisation.", tone: 1, at: 3 }
  ]}
  takeaway="takeaway: lexical nesting becomes fixed hops — unless with or eval hides the names."
  src="src: lesson js-engine/05 · scope chains"
/>
