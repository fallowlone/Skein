...
import rttByGeography from "./infographics/rtt-by-geography.svg?raw";
...
<Infographic
  svg={rttByGeography}
  label="Column chart of round-trip network latency by geography: same-region 1-2 ms, cross-US 60-80 ms, transatlantic 80-100 ms, US to APAC 180-250 ms — the bars escalate roughly a hundredfold."
  caption="Edge pays the full RTT on every sequential query; co-located Node pays the long hop once, then runs each query at sub-millisecond RTT. This asymmetry is what makes the data-locality trap brutal."
  hue="var(--d-infra)"
/>
