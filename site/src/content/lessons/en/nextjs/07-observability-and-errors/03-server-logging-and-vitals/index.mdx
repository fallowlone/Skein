...
import vitalsThresholds from "./infographics/vitals-thresholds.svg?raw";
...
<Infographic
  svg={vitalsThresholds}
  label="A reference list of the three Core Web Vitals — LCP, INP, and CLS — each with its Good and Poor field-p75 thresholds: LCP good under 2.5 s and poor past 4 s, INP good under 200 ms and poor past 500 ms, CLS good under 0.1 and poor past 0.25."
  caption="Aggregate each metric to field p75 per route per deploy, then judge it against these fixed bands. The regressed deploy's 3.4 s LCP sat past Good but short of the 4 s poor line — invisible to any lab budget, which is why it drifted for nine days."
  hue="var(--d-infra)"
/>
