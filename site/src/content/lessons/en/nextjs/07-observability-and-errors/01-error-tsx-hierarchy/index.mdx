...
<Infographic
  svg={resetHonesty}
  label="Two-column comparison of error.tsx reset(): a transient failure (network blip, mid-deploy dependency, timed-out upstream) recovers when reset() is paired with router.refresh(), while a deterministic failure (parsing bug, null DB field, malformed flag) re-throws instantly and turns an auto-retrying boundary into a tight loop, so attempts must be capped and routed to support."
  caption="Same Try again button, opposite truth: reset() re-renders the boundary, which honestly recovers a transient cause but loops forever on a deterministic one — pair it with router.refresh() and cap attempts."
  hue="var(--d-infra)"
/>
