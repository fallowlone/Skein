...
<Infographic
  ...
  label="Two-column comparison of when to use server actions versus route handlers: server actions for in-app form and button mutations, progressive enhancement, opaque per-build ids, sequential execution, and Router Cache refresh; route handlers for external callers, stable documented URLs with explicit methods, GET and non-serializable responses, and parallel requests."
  caption="The boundary of the tool: server actions are for your own React tree's mutations; route handlers serve any external caller, GET, raw responses, or parallel traffic — an opaque per-build action id is not an API contract."
  hue="var(--d-infra)"
/>
