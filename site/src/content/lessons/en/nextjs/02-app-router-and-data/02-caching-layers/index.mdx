...
import diagnoseByLayer from "./infographics/diagnose-by-layer.svg?raw";
...
<Crux>Four stacked caches: Request Memoization, Data Cache, Full Route Cache, Router Cache. Diagnose stale data layer by layer, server→client.</Crux>
...
<FlowDiagram
  label="On-demand invalidation: what revalidateTag reaches from a server action versus from a webhook route handler"
  hue="var(--d-ai)"
  perRow={3}
  nodes={[
    { id: "action", label: "Server action", sub: "revalidateTag('product-42')" },
    { id: "webhook", label: "Route handler", sub: "CMS webhook" },
    { id: "data", label: "Data Cache", sub: "tagged entries purged" },
    { id: "froute", label: "Full Route Cache", sub: "route re-renders" },
    { id: "router", label: "Router Cache", sub: "mutating user's tab" },
    { id: "tabs", label: "Other open tabs", sub: "stale until refresh" },
  ]}
  edges={[
    { from: "action", to: "data", label: "purge" },
    { from: "webhook", to: "data", label: "purge" },
    { from: "data", to: "froute", label: "invalidates" },
    { from: "action", to: "router", label: "same round trip" },
    { from: "froute", to: "tabs", label: "next hard nav" },
  ]}
  caption="Both callers purge the server-side layers. Only a server action can also invalidate the mutating user's client Router Cache in the same round trip — a webhook fixes the server, and open browsers catch up on their next refresh or hard navigation."
/>
...
<Infographic
  svg={diagnoseByLayer}
  label="An ordered four-step diagnosis of stale Next.js data, walked server-first to client-last: step 1 checks whether the route is static (Full Route Cache), step 2 whether the fetch is cached (Data Cache), step 3 whether the mutation invalidates, and step 4 whether only one browser tab is stale (Router Cache)."
  caption="Walk the four steps in order — each question names one culprit layer and its single right lever, so you have the answer before you touch code."
  hue="var(--d-ai)"
/>
...
<StackDiagram
  label="The four cache layers stacked between the browser and the data source, each with its own store and lifetime"
  hue="var(--d-ai)"
  layers={[
    { label: "Router Cache", note: "client, session" },
    { label: "Full Route Cache", note: "server, cleared on deploy" },
    { label: "Data Cache", note: "server, survives deploys" },
    { label: "Request Memoization", note: "one render pass" },
    { label: "Data source", note: "CMS / DB / API" },
  ]}
  caption="A request descends the stack only as far as it must: each layer can answer without consulting the one below. Stale data can hide at any depth — the Router Cache in the user's tab, build-time HTML in the Full Route Cache, or the deploy-proof Data Cache — which is why diagnosis names the layer first."
/>
