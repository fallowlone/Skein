...
<Infographic
  ...
  label="Side-by-side comparison of layout.tsx versus template.tsx: layout.tsx keeps the same mounted instance across soft navigation so client state survives and server code does not re-run, leaving data stale until revalidatePath or router.refresh; template.tsx mounts a new instance every navigation, resetting state and re-running server code, for the rare case where state must reset."
  caption="Same nesting slot, opposite contract: layout persists across navigation (state survives, data goes stale), template remounts every time (state resets, code re-runs). Reach for template only when resetting per navigation is the point."
  hue="var(--d-infra)"
/>
