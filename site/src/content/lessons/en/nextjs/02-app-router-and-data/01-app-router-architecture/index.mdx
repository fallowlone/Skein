...
<StackDiagram
  label="Route segment nesting: the layers above the navigation point persist on soft navigation; only the page level renders again"
  hue="var(--d-ai)"
  layers={[
    { label: "Root layout", note: "persists — html, body" },
    { label: "Shared layout", note: "persists — state survives" },
    { label: "error.tsx", note: "'use client' boundary, reset()" },
    { label: "loading.tsx", note: "Suspense fallback, streams" },
    { label: "page.tsx", note: "re-renders per navigation" },
  ]}
  caption="The fixed nesting order is the segment's contract: layout wraps template, error, loading, page. On soft navigation everything above the changed segment keeps its mounted instance and client state; only the page level re-renders on the server and streams into the Suspense boundary, with error.tsx catching what the page throws."
/>
