...
import hydrationFixes from "./infographics/hydration-fixes.svg?raw";
...
<Infographic
  svg={hydrationFixes}
  label="A decision tree mapping four hydration problems to their fixes: browser-only render to a useEffect swap, a lone Date.now divergence to suppressHydrationWarning or useId, too much client JS pushing INP over 200 ms to a server component, and monolithic blocking to a Suspense boundary."
  caption="Four hydration problems, four different fixes — match the tool to the root cause instead of reaching for one patch."
  hue="var(--d-ai)"
/>
