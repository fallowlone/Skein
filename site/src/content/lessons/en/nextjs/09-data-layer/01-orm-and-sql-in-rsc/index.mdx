...
<Infographic
  ...
  label="Two-way comparison of the 23-widget dashboard: a serial staircase where each widget awaits its own query at ~200 ms per step stacks into ~4.6 s to first byte, versus hoisting with Promise.all or preload so the total collapses to the slowest query at ~200 ms."
  caption="Same 23 widgets, same queries — only the await structure changes the wall-clock: the tree's default serial await stacks 23 steps into ~4.6 s, while hoisting into Promise.all (or preloading) makes the total the slowest query, not the sum."
  hue="var(--d-infra)"
/>
