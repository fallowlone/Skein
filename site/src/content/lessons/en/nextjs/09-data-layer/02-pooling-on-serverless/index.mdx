...
import concurrencyCeiling from "./infographics/concurrency-ceiling.svg?raw";
...
<Infographic
  svg={concurrencyCeiling}
  label="Column chart of the maximum number of concurrent serverless instances reachable before FATAL, for two per-instance pool sizes against the same ~85-connection app budget: pool size 5 caps at 17 instances, while pool size 1 (connection_limit=1) reaches 85."
  caption="On a fixed ~85-connection budget, the per-instance pool is the lever: a server-shaped pool of 5 strands you at 17 concurrent instances, while connection_limit=1 reaches 85 — a 5× jump from one config flag, before any pooler."
  hue="var(--d-infra)"
/>
