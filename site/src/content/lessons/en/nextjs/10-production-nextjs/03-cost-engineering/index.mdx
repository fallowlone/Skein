...
import selfHostCrossover from "./infographics/self-host-crossover.svg?raw";
...
<Infographic
  svg={selfHostCrossover}
  label="Two-way comparison of self-hosting on VMs plus a CDN versus a managed per-request platform: self-host lands near 10-15% of the managed bill on steady traffic and repatriates the image line first, while a managed platform's per-request pricing wins on spiky, low-floor traffic and avoids fleet operations."
  caption="The crossover is traffic shape: decompose first, then move everything, nothing, or just the image line."
  hue="var(--d-infra)"
/>
