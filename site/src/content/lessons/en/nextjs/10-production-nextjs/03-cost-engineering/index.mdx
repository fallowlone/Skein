...
A serverless Next.js bill decomposes into five meters, and each one is driven by a different part of your architecture. **Function GB-hours** — execution time times allocated memory, summed over invocations — is driven by how much of your traffic renders dynamically and how heavy each render is. **Image optimization** bills per transformation: each unique combination of source URL, width, and quality is one resize; repeat requests are cache hits and cost roughly nothing, so the meter counts *unique variants*, not image views. **Bandwidth/egress** is dollars per GB out: client bundles, images, RSC payloads, API responses. **ISR reads and writes** meter operations against the cache store that backs the Full Route and Data Caches. **Edge/middleware invocations** bill per million requests that pass through middleware or edge routes.
