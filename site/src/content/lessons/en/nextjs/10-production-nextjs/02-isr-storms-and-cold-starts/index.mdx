...
import coldStartAnatomy from "./infographics/cold-start-anatomy.svg?raw";
...
<Infographic
  svg={coldStartAnatomy}
  label="Ordered sequence of the four cold-start init phases before the handler runs: provision and extract the bundle, parse and compile the JavaScript (the dominant, size-proportional term), boot the framework, then dial TLS and database connections at 50-300 ms per instance."
  caption="Init runs these phases in order before the first byte; parse-and-compile dominates and scales with bundle size, which is why a trim route finishes in hundreds of milliseconds while a heavy ORM-plus-SDK-plus-markdown route pays 2-5 seconds — and why smaller route bundles is the biggest cold-start lever."
  hue="var(--d-infra)"
/>
