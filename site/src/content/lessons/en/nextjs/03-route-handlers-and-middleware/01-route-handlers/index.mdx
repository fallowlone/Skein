...
import cachingFlip from "./infographics/caching-flip.svg?raw";
...
<Infographic
  svg={cachingFlip}
  label="Side-by-side comparison of the same route.ts GET handler under Next 14 and Next 15: Next 14 caches GET statically at build time (failure: stale snapshots of flags and prices; opt out with force-dynamic), while Next 15 runs GET dynamically per request (failure: per-request database load; opt in with force-static plus revalidate)."
  caption="The same file, inverted default: each version produces the opposite incident — make caching explicit and it survives the upgrade either way."
  hue="var(--d-infra)"
/>
