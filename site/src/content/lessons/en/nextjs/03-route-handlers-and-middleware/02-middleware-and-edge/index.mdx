...
import placementTradeoff from "./infographics/placement-tradeoff.svg?raw";
...
<Infographic
  svg={placementTradeoff}
  label="A two-column comparison: work that belongs in middleware (locale and A/B and legacy redirects, optimistic auth UX, cookie-presence or web-crypto verification, request IDs and bot filtering) versus work that must live deeper in the route handler or data layer (session lookups against the database, per-resource authorization, ORM and TCP drivers and native modules, heavy JWT libraries)."
  caption="Middleware hosts cheap, request-shaped, pre-routing decisions; anything that touches storage is a 35ms-per-request tax multiplied across every match and prefetch — so it belongs in the layer that serves the data."
  hue="var(--d-infra)"
/>
...
<SequenceDiagram
  label="A matched request passes through middleware to the route and data layer — and the CVE-2025-29927 header let a request skip the middleware lane entirely"
  hue="var(--d-ai)"
  actors={["Browser", "middleware()", "Route layer", "Data layer"]}
  messages={[
    { from: "Browser", to: "middleware()", label: "GET /admin" },
    { from: "middleware()", to: "Route layer", label: "next() / rewrite" },
    { from: "Route layer", to: "Data layer", label: "enforcing auth" },
    { from: "Browser", to: "Route layer", label: "spoofed header" },
    { from: "Route layer", to: "Data layer", label: "still checked" },
  ]}
  caption="The normal path runs the matcher and middleware before routing. A request carrying the spoofed x-middleware-subrequest header (CVE-2025-29927) jumped straight to the route layer — which is why the enforcing authorization check lives at the data layer, turning a middleware bypass into a patch-this-week event instead of a breach."
/>
