...
<FlowDiagram
  label="Same request, two placements: edge-near-user pays RTT per query; node-near-data pays it once"
  hue="var(--d-ai)"
  perRow={3}
  nodes={[
    { id: "user", label: "User (Sydney)", sub: "sends request" },
    { id: "edge", label: "Edge fn (Sydney)", sub: "~5ms start" },
    { id: "db", label: "Postgres (us-east)", sub: "~200ms RTT away" },
    { id: "node", label: "Node fn (us-east)", sub: "same DC as DB" },
    { id: "q", label: "4 sequential queries", sub: "×200ms vs ×1ms" },
    { id: "resp", label: "Response", sub: "one user-facing hop" },
  ]}
  edges={[
    { from: "user", to: "edge", label: "30km" },
    { from: "edge", to: "db", label: "×4 = ~800ms" },
    { from: "user", to: "node", label: "200ms once" },
    { from: "node", to: "q", label: "~1ms each" },
    { from: "q", to: "resp", label: "total ~210ms" },
  ]}
  caption="Data gravity in numbers: the edge function starts instantly but pays intercontinental RTT for every one of its four sequential queries; the Node function next to Postgres pays microsecond queries and one long hop to the user. Chatty-with-data routes belong near the data."
/>
