...
import godPackageVsSplit from "./infographics/god-package-vs-split.svg?raw";
...
<Infographic
  svg={godPackageVsSplit}
  label="Two-column comparison of the same monorepo before and after splitting the god package: the shared/utils package gives a 4-percent cache hit rate, 38-minute median PR CI, changes in 70 percent of PRs, and sits under all six builds; splitting by domain and change frequency gives an 85-percent hit rate, 6-minute median PR CI, a billing change that rebuilds only two nodes, and affected-only CI via --filter."
  caption="The numbers move because the boundaries move, not the tooling: the same Turborepo and remote cache go from a 4-percent hit rate and 38-minute PRs to 85 percent and 6 minutes once the god package is split by domain and change frequency."
  hue="var(--d-infra)"
/>
