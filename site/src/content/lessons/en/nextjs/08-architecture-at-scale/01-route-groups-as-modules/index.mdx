...
import reloadTax from "./infographics/reload-tax.svg?raw";
...
<Infographic
  svg={reloadTax}
  label="A side-by-side comparison of two navigation modes in one Next.js app: client navigation within the same root layout costs about 100 ms, patches the tree below the shared layout, and preserves client state; a full document reload across root layouts costs roughly 0.5 to 1.5 seconds on mid-tier mobile, replaces the document, and loses all client state."
  caption="Same app, two navigation modes: staying inside one root layout is a ~100 ms patch that keeps client state; crossing root layouts is a full document reload — ~0.5–1.5 s and every piece of client state gone. Cut root layouts only along seams crossed once a session, like login."
  hue="var(--d-infra)"
/>
