...
<FlowDiagram
  label="Three candidate boundaries; only the one attached to the data is authoritative"
  hue="var(--d-ai)"
  perRow={3}
  nodes={[
    { id: "req", label: "Request", sub: "cookie, maybe hostile headers" },
    { id: "mw", label: "Middleware", sub: "optimistic redirect — bypassable" },
    { id: "layout", label: "Layout check", sub: "persists, renders in parallel" },
    { id: "page", label: "Page render", sub: "fetches regardless" },
    { id: "dal", label: "DAL verifySession", sub: "cache(): 1 lookup/request" },
    { id: "data", label: "Row + authz", sub: "ownerId === session.userId" },
  ]}
  edges={[
    { from: "req", to: "mw", label: "can skip (CVE)" },
    { from: "mw", to: "layout", label: "first load only" },
    { from: "layout", to: "page", label: "no gating" },
    { from: "page", to: "dal", label: "every read" },
    { from: "dal", to: "data", label: "enforced" },
  ]}
  caption="Middleware redirects fast but can be skipped — CVE-2025-29927 proved the class. A layout check runs once and never gates the page's parallel fetches. The Data Access Layer is unavoidable by construction: every query starts with a memoized verifySession and an instance-level ownership check."
/>
