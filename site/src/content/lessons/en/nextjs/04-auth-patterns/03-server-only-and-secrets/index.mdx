...
<Infographic
  svg={envVarRegimes}
  label="Side-by-side comparison of two environment-variable regimes in Next.js: a plain process.env.SECRET server var (read at runtime, undefined in a client bundle, rotatable without a rebuild) versus a NEXT_PUBLIC_SECRET public var (inlined as a string literal at build time, readable by every visitor, rotatable only by rebuild and redeploy, compromised once shipped)."
  caption="Same code, one prefix apart: the plain server var only ever crashes when it reaches the client; the NEXT_PUBLIC_ var is baked into the bundle as a literal — public to every visitor, unrotatable without a rebuild, and compromised the moment it ships."
  hue="var(--d-infra)"
/>
