...
import frameworkVsCode from "./infographics/framework-vs-code.svg?raw";
...
<Infographic
  svg={frameworkVsCode}
  label="Two-column comparison: the framework defends provenance only — SameSite=Lax cookies, the Origin-versus-Host check on every action, and encrypted closure values — while only your code defends the application with verifySession against anonymous calls, a rate limit against credential stuffing and denial-of-wallet, zod-parsing against mass assignment, and a row-ownership check against IDOR."
  caption="The framework stops the attacks aimed at the framework; only your in-action pipeline stops the attacks aimed at the application — IDOR is the authentic request CSRF never sees."
  hue="var(--d-ai)"
/>
