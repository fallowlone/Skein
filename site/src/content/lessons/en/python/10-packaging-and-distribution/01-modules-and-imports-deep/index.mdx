...
import circularImportFate from "./infographics/circular-import-fate.svg?raw";
...
<Infographic
  svg={circularImportFate}
  label="Side-by-side comparison of two import forms inside a circular import: import a binds the half-built module and defers attribute access to call time, so it survives; from a import helper demands the attribute mid-execution while a is half-initialized, so it raises ImportError on the partially initialized module."
  caption="Both modules are registered in sys.modules before their bodies run; the only difference is when each form demands the name — import a waits for call time and survives, from a import helper demands it mid-init and dies."
  hue="var(--d-infra)"
/>
