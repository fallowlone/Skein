...
import hintVocabulary from "./infographics/hint-vocabulary.svg?raw";
...
<Infographic
  svg={hintVocabulary}
  label="Taxonomy of Python's modern type-hint vocabulary branching from one checkable contract into five tools: X | Y unions, generic built-ins, TypedDict, Literal, and Annotated, each with the loose pattern it pins down."
  caption="Each tool pins one loose runtime pattern — an anonymous dict, a stringly-typed flag, a maybe-None return — into a contract a checker can enforce, with zero runtime cost."
  hue="var(--d-infra)"
/>
