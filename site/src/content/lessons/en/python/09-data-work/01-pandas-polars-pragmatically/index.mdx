...
import whenEachWins from "./infographics/when-each-wins.svg?raw";
...
<Infographic
  svg={whenEachWins}
  label="Side-by-side decision: keep pandas for eager execution, ~5x object-string memory, silent int-to-float promotion, and the scikit-learn/notebook ecosystem on small data; reach for polars for lazy pushdown that reads 2 of 40 columns, indexless Arrow columns, a strict multi-threaded Rust engine running 5-50x faster, and scheduled pipelines over data near or above RAM."
  caption="The pragmatic split: pandas owns ecosystem-and-small-data, polars owns pipeline-scale — and the pitfalls (memory tax, silent promotion, the apply cliff) sit on the pandas side."
  hue="var(--d-infra)"
/>
