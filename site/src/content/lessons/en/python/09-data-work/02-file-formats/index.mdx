...
import formatSize from "./infographics/format-size.svg?raw";
...
<Infographic
  svg={formatSize}
  label="Column chart of on-disk size for the same 10 million-row, ~20-column table: CSV about 1200 MB, gzipped CSV about 350 MB, Parquet with zstd about 200 MB."
  caption="Same table, three formats: CSV ~1.2 GB collapses to ~200 MB as Parquet+zstd — roughly 6x smaller, and the typed schema now travels inside the file instead of being guessed on every read."
  hue="var(--d-infra)"
/>
