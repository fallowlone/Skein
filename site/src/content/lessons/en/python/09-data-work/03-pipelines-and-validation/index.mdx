...
<Infographic
  svg={idempotentWrite}
  label="Side-by-side comparison of two write shapes. Blind append: a retry doubles the day's rows, there is nothing to roll back on object or file sinks, recovery means forensic deduplication, and backfill is an emergency script. Idempotent write: overwrite the window partition or merge on a business key, so a retry replaces state instead of adding to it, and backfill becomes a parameter via the watermark."
  caption="Blind append plus a retry equals duplicates; keying the output by processing window and overwriting it makes the rerun replace state, not extend it — the same structure that turns backfill from an incident into a parameter."
  hue="var(--d-infra)"
/>
