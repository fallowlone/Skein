...
<Infographic
  svg={scopeTradeoff}
  label="Side-by-side comparison of function scope versus session scope for one Postgres testcontainer across 300 integration tests: function scope rebuilds per test (~40 min/run, full isolation, no order-dependent bugs); session scope builds once (~8s/run, the same object handed to every test, order-dependent failures unless wrapped in a function-scoped transaction)."
  caption="Scope is the cache key: session scope is ~300× faster to set up but hands one object to every test — the order-dependent failure class. The disciplined fix is an immutable broad-scoped core under a function-scoped transaction rollback."
  hue="var(--d-infra)"
/>
