...
import connectionBudget from "./infographics/connection-budget.svg?raw";
...
<Infographic
  svg={connectionBudget}
  label="Column chart of connection counts: a lifespan pool totals 80 connections (4 workers × 2 pods × 10), Postgres allows max_connections = 100, but a pool per request at 50 rps demands 500 connections — five times past the ceiling."
  caption="The deliberate design (80) sits under the 100 ceiling; a pool built inside a dependency demands 50 rps × 10 = 500 connections and breaches max_connections in the first second. That gap is the canonical incident."
  hue="var(--d-infra)"
/>
