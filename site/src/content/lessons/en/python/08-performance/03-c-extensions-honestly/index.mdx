...
import boundaryTax from "./infographics/boundary-tax.svg?raw";
...
<Infographic
  svg={boundaryTax}
  label="Column chart of the Hook's three runs of the same hundred-million float-op job: pure Python takes 90 seconds, the same kernel called per element through ctypes takes 110 seconds (slower), and one vectorized call that crosses the boundary once takes 0.9 seconds."
  caption="Adding C per element made the job slower — the boundary tax, paid ten million times, swallowed the C win. Moving the loop across the boundary once is the ~100x, and the reason granularity, not language, decides the outcome."
  hue="var(--d-infra)"
/>
