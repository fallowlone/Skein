...
<Infographic
  ...
  label="Column chart of total wall-clock time for three I/O calls of one second each: awaiting them sequentially takes 3 seconds (the sum of latencies), while wrapping them as Tasks via gather takes about 1 second (the max)."
  caption="Awaiting coroutines one by one pays the sum of their latencies; wrapping them as Tasks (create_task / gather) pays only the max — that 3x-to-1x gap is exactly where concurrency begins."
  hue="var(--d-infra)"
/>
...
<SequenceDiagram
  label="The await handshake over one loop iteration: the loop resumes a coroutine, the coroutine suspends on a Future at its await, the loop polls the selector, and socket readiness schedules the resume"
  hue="var(--d-systems)"
  actors={["Coroutine", "Event loop", "Selector"]}
  messages={[
    { from: "Event loop", to: "Coroutine", label: "resume frame" },
    { from: "Coroutine", to: "Event loop", label: "await read: suspend" },
    { from: "Event loop", to: "Selector", label: "poll epoll/kqueue" },
    { from: "Selector", to: "Event loop", label: "socket ready" },
    { from: "Event loop", to: "Coroutine", label: "resume with data" },
  ]}
  caption="await is the only door back to the loop: a synchronous call inside the running step never returns control, so the selector is never polled and every timer, accept, and other coroutine freezes. Delegate blocking work via run_in_executor / to_thread and await the Future instead."
/>
