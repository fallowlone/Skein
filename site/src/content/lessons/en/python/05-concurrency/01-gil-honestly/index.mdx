...
<FlowDiagram
  label="A thread's path through the GIL: acquire, run bytecode until the 5 ms drop request or a blocking call, release, repeat"
  hue="var(--d-systems)"
  perRow={3}
  nodes={[
    { id: "want", label: "Thread ready", sub: "wants to run" },
    { id: "gil", label: "Acquire GIL", sub: "one holder/process" },
    { id: "run", label: "Run bytecode", sub: "refcounts safe" },
    { id: "tick", label: "5 ms drop request", sub: "setswitchinterval" },
    { id: "io", label: "Blocking I/O / C ext", sub: "releases GIL" },
    { id: "rel", label: "Release", sub: "OS picks next" },
  ]}
  edges={[
    { from: "want", to: "gil", label: "contend" },
    { from: "gil", to: "run", label: "hold" },
    { from: "run", to: "tick", label: "timer" },
    { from: "run", to: "io", label: "syscall" },
    { from: "tick", to: "rel", label: "drop" },
    { from: "io", to: "rel", label: "drop" },
  ]}
  caption="I/O and GIL-releasing C extensions overlap for real; pure bytecode serializes. Free-threaded 3.13+ deletes the lock and replaces it with biased refcounting plus per-object locks."
/>
