...
<Infographic
  svg={bridgeTradeoff}
  label="Side-by-side comparison of two ways a sync server reaches an async SDK: asyncio.run per request builds and tears down a fresh loop every call, giving 0% connection reuse, a TLS handshake on every request, 3x latency and +40% CPU on handshakes; a long-lived loop in a daemon thread with run_coroutine_threadsafe reuses the pool and TLS sessions, blocks only the caller, and drops latency below the old sync client."
  caption="Same coroutine, opposite outcomes: the per-call loop rebuilds every pool and TLS session, while one long-lived loop crossed with run_coroutine_threadsafe is the one-line server fix."
  hue="var(--d-infra)"
/>
