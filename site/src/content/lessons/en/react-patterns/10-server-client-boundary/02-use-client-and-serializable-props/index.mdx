...
status: ready
summary: '''use client'' marks a module and its whole import subtree as client; props from a Server to a Client Component must be serializable, so data crosses the boundary downward as serializable props while server content is composed into client shells via children — not by importing across.'
