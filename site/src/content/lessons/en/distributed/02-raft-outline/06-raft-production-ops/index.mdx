...
<Infographic
  ...
  label="A versus comparison: the naive per-group scheme sends 50,000 groups times one heartbeat every 50 ms — 1,000,000 heartbeats per second, scaling with group count; the coalesced scheme sends 10 physical packets per 50 ms, 200 packets per second total, scaling with node count instead."
  caption="One physical packet per node pair carries every group's heartbeat in a compact bitmap, so traffic tracks node count, not group count — 5,000x fewer packets is what makes Multi-Raft viable at 50,000 groups."
  hue="var(--d-infra)"
/>
