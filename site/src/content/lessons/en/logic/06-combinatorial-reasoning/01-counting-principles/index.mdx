...
When you see a nested loop, ask yourself: is each level an independent "and then"? If so, you can multiply the per-level counts instead of running the code. Code already speaks this rule fluently — every nested loop is a multiplication:
