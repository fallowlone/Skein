...
<Infographic
  svg={offsetCliff}
  label="Column chart of rows read to return one 20-row page under OFFSET pagination: Page 1 reads 20 rows, Page 1,000 reads 20,020, Page 2,500 reads 50,020, and Page 5,000 reads 100,020 — the bars climb linearly with page depth, while keyset pagination stays flat at 20 rows per page."
  caption="OFFSET reads offset+20 rows and discards the offset, so cost grows linearly with depth — Page 5,000 reads 100,020 rows for the same 20-row page. Keyset stays flat at 20 at any depth."
  hue="var(--d-infra)"
/>
