...
<Infographic
  svg={sargableLatency}
  label="Column chart of query latency on a 20-million-row users table: the sargable predicate email = $1 runs as a 2 ms index scan, while the non-sargable lower(email) = $1 runs as a 1.9 second sequential scan."
  caption="Same table, same index, same data — only the predicate shape changed. Keeping the column bare is a 2 ms index scan; wrapping it in lower() is a 1.9 s full scan, the lesson's ~900× regression."
  hue="var(--d-data)"
/>
