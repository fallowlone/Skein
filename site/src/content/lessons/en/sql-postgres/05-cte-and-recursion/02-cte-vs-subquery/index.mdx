...
import optimizationFence from "./infographics/optimization-fence.svg?raw";
...
<Infographic
  svg={optimizationFence}
  label="Side-by-side comparison of how the Postgres planner treated a pre-PG12 CTE versus an equivalent inline subquery in FROM: the CTE was an unconditional optimization fence, materialized whole with no predicate pushdown, so an outer WHERE tenant_id = 7 scanned far more rows; the inline subquery was flattened, let the predicate push down to an index, and filtered early — the behavior PG12 made the CTE default via NOT MATERIALIZED."
  caption="The fence is why CTE-vs-subquery was substance, not style: identical logic produced different plans, and PG12's NOT MATERIALIZED default closed the gap."
  hue="var(--d-infra)"
/>
