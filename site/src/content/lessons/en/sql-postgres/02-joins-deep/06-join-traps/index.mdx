...
import fanoutFix from "./infographics/fanout-fix.svg?raw";
...
<Infographic
  svg={fanoutFix}
  label="Side-by-side comparison of two approaches to the same orders, items, and payments data. Joining the two one-to-many children then aggregating produces 3 times 2 equals 6 rows per order, so SUM(total) reports 600 against a true 100, with no error raised, and it even passes in dev where one item times one payment is one row. Pre-aggregating each child first yields one row per order per child, SUM(total) equals the correct 100, the joins are one-to-one so they cannot fan out, and the pattern scales to any number of children."
  caption="Pre-aggregating each child to one row per parent turns the SUM from a silent 6× over-count (600) into the correct 100 — the join can no longer multiply."
  hue="var(--d-data)"
/>
