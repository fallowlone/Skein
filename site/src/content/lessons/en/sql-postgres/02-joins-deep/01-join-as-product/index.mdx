...
import productVsFiltered from "./infographics/product-vs-filtered.svg?raw";
...
<Infographic
  svg={productVsFiltered}
  label="Side-by-side comparison: a CROSS JOIN keeps the full Cartesian product — no ON predicate, so the row count multiplies (|A| × |B|, e.g. 3 users × 4 orders = 12 rows) and a missing predicate silently lands here as the 4.2-billion-row outage; an INNER JOIN ON a predicate applies that predicate as a filter on the same product, identical to CROSS JOIN ... WHERE p, so its output is the surviving matching pairs (4 rows for 3 users and 4 orders), never simply |A| or |B|."
  caption="An inner join is the same product as a cross join, one ON filter apart — which is why a missing predicate silently degrades to the full product and an outage."
  hue="var(--d-data)"
/>
