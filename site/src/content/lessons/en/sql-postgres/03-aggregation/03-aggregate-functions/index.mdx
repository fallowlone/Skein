...
<Infographic
  svg={filterOnePass}
  label="Column chart comparing wall time on a 12M-row orders table: one FILTER query that reads every row once takes about 850ms, while three separate WHERE queries that scan the table three times take about 2400ms."
  caption="Same three counts, two plans: FILTER routes each row to the aggregates it matches in a single pass (~850ms), versus three sequential scans of the whole table (~2.4s)."
  hue="var(--d-infra)"
/>
