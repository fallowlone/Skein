...
<Infographic
  svg={rowsVsRange}
  label="Side-by-side comparison of two frame modes. ROWS counts physical rows: 1 PRECEDING is literally one row back, each row adds its own amount in order, it is correct on tied ORDER BY values, generally faster (no scan-ahead for peers), and works with any ORDER BY. RANGE operates on ORDER BY values: tied rows are peers that enter and leave the frame together, so running totals show the same peer-inclusive total on every tied row; it is the silent default for an inner ORDER BY with no frame, and is required for value-based frames like INTERVAL '7 days' PRECEDING on a single numeric or date column."
  caption="The split is ties: ROWS counts physical positions and never overcounts, while the default RANGE groups ORDER BY peers — write ROWS explicitly for any running total."
  hue="var(--d-infra)"
/>
