...
<Infographic
  svg={typeDerivation}
  label="Hierarchy tree rooted at the one Comment zod schema: every other type — type Comment via z.infer, NewComment via Comment.pick, the CreateResult union, Paginated via makePage, and AppRouter via typeof appRouter, which the client infers Comment[] from — branches off it as a derived child."
  caption="One schema is the only place the shape is written; z.infer, Pick, typeof, and typeof appRouter are the branches that derive every other layer from it — nothing is duplicated by hand."
  hue="var(--d-infra)"
/>
