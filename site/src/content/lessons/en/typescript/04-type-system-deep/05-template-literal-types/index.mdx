...
import parserVsCodegen from "./infographics/parser-vs-codegen.svg?raw";
...
<Infographic
  svg={parserVsCodegen}
  label="Two-way comparison of a template-literal type parser versus a codegen build step: the template earns zero-runtime safety when the string is the contract but caps out at the ~100k union limit and silently falls back to string past ~50 recursion levels, while generated flat types compile instantly and stay debuggable."
  caption="Reach for templates when the string itself is the contract; reach for codegen when the string needs parsing — generated flat types are greppable, hover-clean, and compile instantly."
  hue="var(--d-infra)"
/>
