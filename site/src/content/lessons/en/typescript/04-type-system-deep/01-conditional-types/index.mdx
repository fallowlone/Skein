...
<Infographic
  svg={condVsUnion}
  label="Two-column comparison: a conditional type makes the return type track the input at zero runtime cost but bills come due at compile time and in opaque errors near the 100,000-member union cap, while a discriminated union plus a plain if is cheaper to read and compile with actionable errors yet pays one branch at runtime."
  caption="Reach for a conditional type only when the return type genuinely varies with the input; for a single runtime decision a discriminated union plus a plain if compiles faster and fails legibly."
  hue="var(--d-infra)"
/>
