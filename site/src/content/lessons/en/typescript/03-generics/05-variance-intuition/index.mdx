...
<Infographic
  ...
  label="Side-by-side comparison of the same on(handler: (e: Event) => void) slot written two ways. The method-shorthand form makes the parameter bivariant even under strictFunctionTypes, so a narrow (e: MouseEvent) handler is accepted with no error and e.button is undefined at runtime. The property-function form keeps the parameter contravariant, so the same narrow handler is rejected at compile time."
  caption="The only difference is the declaration form, but the variance flips: the method shorthand silently accepts a too-narrow callback, the property-function form catches it. Reach for the property form when the parameter must actually be checked."
  hue="var(--d-infra)"
/>
