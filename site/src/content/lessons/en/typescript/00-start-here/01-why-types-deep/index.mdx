...
<Infographic
  svg={refactorSafety}
  label="Side-by-side comparison of refactoring one function signature across 23 call sites. Plain JavaScript (left): grep and fix by hand, a missed site silently passes undefined and crashes in production weeks later. TypeScript (right): a whole-program check makes every stale call site a compile error at once, caught before the code runs."
  caption="Changing a contract turns the type checker into a whole-program search: every stale call site becomes a compile error at once, instead of a landmine you grep for by hand."
  hue="var(--d-frontend)"
/>
