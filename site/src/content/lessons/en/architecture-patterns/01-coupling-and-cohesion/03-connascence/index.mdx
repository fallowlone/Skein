...
<Crux>Connascence names the kind of dependency two components share — static (compile-time) or dynamic (runtime) — plus how strong, how local, and how widespread it is.</Crux>
...
<FlowDiagram
  ...
  edges={[
    ...
    { from: "com", to: "coa", label: "stronger" },
  ]}
  caption="Static connascence is detectable at compile time. CoN is weakest — a rename refactor fixes it mechanically. CoM and CoA are dangerous because the coupling is invisible to the type system and produces silent failures when one side changes."
/>
...
<FlowDiagram
  ...
  nodes={[
    { id: "type", label: "Type (strength)", sub: "CoN < CoT < CoM < CoA < CoE < CoV < CoI" },
...
<KeyTakeaway>Connascence names why two components must change together — from CoN (a shared name) through CoM (a shared convention) to CoV (shared runtime state). Rank refactors by: weaken the connascence type first, then localize it, then reduce its degree. Strong connascence across service boundaries is the most dangerous combination.</KeyTakeaway>
...
<Recap lang="en">
Connascence is the vocabulary that unifies coupling and cohesion. Two components are connascent when a change to one requires a corresponding change to the other. The classification names why: Connascence of Name (shared identifier), Type (shared type declaration), Meaning (shared value convention), Algorithm (shared implementation), Execution (shared runtime sequence), Timing (shared time window), Values (shared runtime data), Identity (shared object reference). The first four are static — detectable at compile time. The last four are dynamic — detectable only at runtime, and therefore more dangerous.
