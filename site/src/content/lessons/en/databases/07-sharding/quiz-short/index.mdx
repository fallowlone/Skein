---
slug: quiz-short
lang: en
track: databases
unit: 07-sharding
order: 100
title: "Short Answer Quiz"
summary: "Short Answer Quiz for unit "Sharding PostgreSQL: shard keys, hot-shard failure mode, partitioning vs Citus""
estMin: 30
status: ready
prereqs: []
concepts: []
sources: []
---
import Hook from "~/components/lesson/Hook.astro";
import Goal from "~/components/lesson/Goal.astro";
import Step from "~/components/lesson/Step.astro";
import WorkedExample from "~/components/lesson/WorkedExample.astro";
import Check from "~/components/lesson/Check.astro";
import Recap from "~/components/lesson/Recap.astro";
import Inset from "~/components/lesson/Inset.astro";
import PracticeSet from "~/components/lesson/PracticeSet.astro";
import Quiz from "~/components/pedagogy/Quiz.astro";

<Hook>
This is a quiz-short for the Sharding PostgreSQL: shard keys, hot-shard failure mode, partitioning vs Citus unit.
</Hook>

<Goal lang="en>
After this lesson you will be able to apply the concepts from the Sharding PostgreSQL: shard keys, hot-shard failure mode, partitioning vs Citus unit.
</Goal>

<Step n={1}>
This lesson contains practice questions and exercises to reinforce your learning.
</Step>

<Recap lang="en>
Review the key concepts from this lesson and prepare for the next unit.
</Recap>

