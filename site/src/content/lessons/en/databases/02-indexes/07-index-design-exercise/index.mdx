...
<Infographic
  svg={indexBudget}
  label="Column chart of the five-index budget for a 100M-row ticketing table (~40 GB): the two partial composites are ~0.6 GB each, the recent-feed composite ~1.5 GB, the UNIQUE ticket index ~2 GB, and the GIN tsvector ~4 GB — about 8.6 GB total."
  caption="The partial composites (WHERE status IN ('open','in_progress')) are the cheapest entries because they index only the hot 20% of rows; GIN tsvector dominates. The five sum to ~8.6 GB — comfortably under 20% of the ~40 GB table."
  hue="var(--d-infra)"
/>
