...
summary: "Pool N workers sized to hardwareConcurrency − 1, dispatch jobs via a priority queue with backpressure, wrap with Comlink for ergonomics — but postMessage task-hops still cost, and every worker thread needs its own telemetry."
estMin: 16
status: ready
lessonType: topic
level: senior
prereqs: ["browser/04-workers/02-web-worker-mechanics", "browser/04-workers/03-structured-clone-and-transfer"]
deepensInto: []
spiral: ["encapsulation"]
concepts: ["worker-pool", "backpressure", "priority-queue", "Comlink", "postMessage-latency", "task-hop", "worker-observability", "DOM-bottleneck"]
sources:
