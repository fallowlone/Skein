...
import heapBudget from "./infographics/heap-budget.svg?raw";
...
<Infographic
  svg={heapBudget}
  label="Donut chart splitting a 512 MB container memory limit into a 384 MB old-space heap (about 75%) and 128 MB of off-heap headroom for Buffers, native addons, stacks, and the young generation."
  caption="The container limit covers the whole process, not just the heap — so --max-old-space-size sits at ~75% (384 of 512 MB), leaving the 128 MB the rest of the runtime needs. Match the heap to 512 and total RSS overruns the cgroup, OOMKilling you at 137 again."
  hue="var(--d-infra)"
/>
