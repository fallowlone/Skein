...
<Infographic
  svg={startupTax}
  label="Line chart: a transpile-on-load hook's extra startup time grows linearly with module count — about 5 seconds at 100 files, 10 at 200, 15 at 300, and 20 at 400, at roughly 50 ms per file, paid on every node start."
  caption="The hook runs per module, so the transpile cost stacks linearly across the dependency tree — 12-20 extra seconds for a 400-file app, every cold start. That per-invocation tax is why you build for prod and keep hooks for dev."
  hue="var(--d-infra)"
/>
