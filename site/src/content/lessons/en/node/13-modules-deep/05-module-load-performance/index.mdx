...
import coldStartLevers from "./infographics/cold-start-levers.svg?raw";
...
<Infographic
  svg={coldStartLevers}
  label="A ranked list of four cold-start optimizations, cheapest first: lazy require (skips a rare-path dep), bundling (kills resolution I/O, ~3s to under 1s), NODE_COMPILE_CACHE (reuses warm bytecode, cutting the 20-40% compile slice by an order of magnitude), and a startup snapshot (freezes the post-init heap to skip eval) — each with the phase it targets and its hard catch."
  caption="Match the lever to the phase your profile names, escalating from cheapest to heaviest: lazy require → bundle → compile cache → snapshot. Each cuts a different phase, so the wrong lever buys nothing."
  hue="var(--d-infra)"
/>
