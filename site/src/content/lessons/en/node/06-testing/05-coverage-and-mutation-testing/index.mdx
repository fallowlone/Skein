...
import coverageVsMutation from "./infographics/coverage-vs-mutation.svg?raw";
...
<Infographic
  svg={coverageVsMutation}
  label="Column chart of the same module measured two ways: line coverage reads 100%, but mutation score is only 60% — the 40-point gap is injected bugs that survive because no test asserts on that behavior."
  caption="Same code, two metrics. Coverage says 100% (every line ran); mutation score says 60% (40% of injected bugs survive untested). That 40-point gap is the behavior your suite executes but never verifies."
  hue="var(--d-infra)"
/>
