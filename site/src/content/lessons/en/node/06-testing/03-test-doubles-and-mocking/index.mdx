...
import stateVsBehavior from "./infographics/state-vs-behavior.svg?raw";
...
<Infographic
  svg={stateVsBehavior}
  label="Two-column comparison of the verification-style axis: the state-verification column groups stub and fake, which return canned values or hold real state and assert on the outcome, so they survive refactors; the behavior-verification column groups spy and mock, which record calls or carry expectations and assert on the interaction, so they are brittle and pinned to the implementation."
  caption="The senior line: stubs and fakes assert on outcome and survive refactors; spies and mocks assert on interaction and pin the test to the current implementation — pick the verification style before you pick the double."
  hue="var(--d-infra)"
/>
