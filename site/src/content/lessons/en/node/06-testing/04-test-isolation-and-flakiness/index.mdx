...
<Infographic
  ...
  label="An ascending three-rung ladder of parallel database isolation strategies ordered by per-test setup cost: transaction-per-test rollback at about 1 to 3 milliseconds, schema or database per worker with truncation at tens of milliseconds, and an ephemeral container at seconds — each rung buying more fidelity for more time."
  caption="Each rung up the ladder pays roughly an order of magnitude more time and buys more fidelity: rollback can't test a real COMMIT, truncate can, containers pin the exact engine. Take the cheapest rung that still exercises what you assert."
  hue="var(--d-infra)"
/>
