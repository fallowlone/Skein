...
import sessionVsJwt from "./infographics/session-vs-jwt.svg?raw";
...
<Infographic
  svg={sessionVsJwt}
  label="Side-by-side comparison of the two auth models. Opaque session: revocable instantly via DEL session:id, costs a per-request store lookup, keeps state server-side with only a 128-bit random id in the cookie, and lives in an HttpOnly cookie XSS cannot read. Self-contained JWT: cannot be un-issued so it is valid until exp, needs no lookup and scales flat, carries its claims in the signed header.payload.signature, and sits in localStorage that XSS can exfiltrate."
  caption="The two models fail in opposite directions: the session trades a store lookup for instant revocation and XSS-safety, while the JWT trades revocability away for stateless scale."
  hue="var(--d-infra)"
/>
