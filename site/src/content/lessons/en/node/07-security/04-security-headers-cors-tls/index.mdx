...
A security response header is a directive you send on every response that reconfigures the browser's own defenses. It costs nothing at runtime and is the cheapest layer you own, but each header is paired with a specific attack — set the wrong one and you get a false sense of safety.
