...
<Infographic
  svg={exitCodes}
  label="An escalation ladder of process exit codes decoded as 128 + N: 0 is a clean exit where the loop drained on its own; 130 = 128 + 2 is SIGINT (Ctrl-C); 143 = 128 + 15 is an unhandled SIGTERM; and 137 = 128 + 9 is SIGKILL, the forced kill after the grace period expired."
  caption="The exit code is your contract with the supervisor: decode 128 + N to read which signal ended the process. 137 is never a code crash — it is the SIGKILL hammer after the grace period ran out."
  hue="var(--d-infra)"
/>
