...
import transformMemory from "./infographics/transform-memory.svg?raw";
...
<Infographic
  svg={transformMemory}
  label="Side-by-side comparison of two Transform authoring strategies. Pushing per chunk in _transform keeps working memory at roughly 3×16KB across a three-stage pipeline, so a 4GB input streams in about 48KB. Buffering every chunk to emit in _flush holds the entire dataset in RAM, so a 4GB input costs 4GB and OOMs."
  caption="Same job, two memory profiles: push-as-you-go stays flat at ~48KB across the pipeline; buffer-everything-in-_flush scales with input size and a 4GB stream costs 4GB then OOMs."
  hue="var(--d-infra)"
/>
