...
import streamTypes from "./infographics/stream-types.svg?raw";
...
<Infographic
  svg={streamTypes}
  label="Taxonomy of Node's four stream types branching from a root: Readable (a source you pull from — file read, HTTP request body, socket), Writable (a sink you push to — file write, HTTP response, socket; write() returns false past highWaterMark), Duplex (both sides independent — a TCP socket), and Transform (a Duplex that maps input to output — gzip, cipher, CSV parser)."
  caption="Four stream types, one mental model: Readable pulls, Writable pushes (and signals backpressure via write()'s return), Duplex does both with independent buffers, and Transform is a Duplex that maps input to output — the streaming map."
  hue="var(--d-infra)"
/>
