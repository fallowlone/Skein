...
import renderBudgetBreach from "./infographics/render-budget-breach.svg?raw";
...
<Infographic
  svg={renderBudgetBreach}
  label="Column chart of render time in milliseconds: a laptop renders the grid in 90 ms, the waitFor default budget is 1,000 ms, and a 2-vCPU CI runner takes 1,300 ms — the only bar that exceeds the budget."
  caption="Same test, same assertions — only the machine differs. The CI render (1.3 s) overshoots the 1 s waitFor budget while the laptop render (90 ms) sits far under it, which is why the test is green locally and red in CI. The fix is a deliberate asyncUtilTimeout for CI hardware, not a sleep."
  hue="var(--d-frontend)"
/>
