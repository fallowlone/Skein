...
<Infographic
  svg={bundleWeight}
  label="Column chart comparing client JavaScript per article route in kilobytes: the SPA build ships every library at roughly 1100 kB, while the RSC build ships only output at roughly 210 kB."
  caption="The same markdown, highlighter and i18n libraries run on the server under RSC — about 900 kB of client JavaScript per article route simply never ships."
  hue="var(--d-infra)"
/>
