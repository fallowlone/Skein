...
<Infographic
  svg={coverageGap}
  label="Donut chart of WCAG failure coverage: axe-core automates roughly a third (statically computable facts), while the remaining two thirds need a manual screen-reader pass and interaction tests."
  caption="Deque markets 57% by volume, but independent audits land near a third — a green axe run is a floor for that third, never a certificate for the other two thirds."
  hue="var(--d-frontend)"
/>
