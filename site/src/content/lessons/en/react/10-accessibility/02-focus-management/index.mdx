...
import pageLoadVsRoute from "./infographics/page-load-vs-route.svg?raw";
...
<Infographic
  svg={pageLoadVsRoute}
  label="Two-column comparison of the three accessibility jobs a full page load does automatically — focus reset to the new document, document title announced, reading order from a known start — against a SPA route change, which does none: focus stays stranded on stale DOM or body, nothing is announced because a DOM diff is not a navigation event, and the next Tab starts from nowhere."
  caption="A full page load hands assistive tech three signals for free; a client-side route change hands it none — so you reproduce all three by hand, starting with focusing the new heading via tabIndex -1."
  hue="var(--d-frontend)"
/>
