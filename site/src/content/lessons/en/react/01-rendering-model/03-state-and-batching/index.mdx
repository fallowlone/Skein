...
<FlowDiagram
  label="setState calls append to a per-hook queue; a batch flushes as one render and one commit, while flushSync forces a synchronous early flush"
  hue="var(--d-frontend)"
  perRow={3}
  nodes={[
    { id: "handler", label: "Handler / callback", sub: "setState × N" },
    { id: "queue", label: "Update queue", sub: "values + updaters" },
    { id: "batch", label: "Batch", sub: "one flush per tick" },
    { id: "render", label: "Render", sub: "reduce queue in order" },
    { id: "commit", label: "Commit", sub: "DOM updated once" },
    { id: "flush", label: "flushSync", sub: "sync render+commit" },
  ]}
  edges={[
    { from: "handler", to: "queue", label: "enqueue" },
    { from: "queue", to: "batch", label: "collect" },
    { from: "batch", to: "render", label: "schedule" },
    { from: "render", to: "commit", label: "apply" },
    { from: "handler", to: "flush", label: "escape hatch" },
    { from: "flush", to: "commit", label: "force now" },
  ]}
  caption="State updates are messages, not writes: values and updater functions queue on the fiber, the batch flushes once per tick (everywhere in React 18), and the queue is reduced in order at render time. flushSync trades batching away for an immediately committed DOM."
/>
