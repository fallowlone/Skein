...
import catchVsBlindSpots from "./infographics/catch-vs-blind-spots.svg?raw";
...
<Infographic
  svg={catchVsBlindSpots}
  label="Two-column comparison: a boundary catches render errors, lifecycle methods, and constructors; four categories slip past it — event handlers, async code, server-side rendering, and the boundary's own fallback."
  caption="A boundary only catches the render path (render, lifecycles, constructors). The four blind spots — handlers, async, SSR, and the boundary itself — each need their own countermeasure."
  hue="var(--d-infra)"
/>
