...
<Infographic
  svg={channelDecision}
  label="A 2x2 decision matrix on two axes — how often the value changes (rare vs hot) and how widely it is read (wide fanout vs narrow): rare and wide read goes to context, rare across 2-3 hops to props, hot and wide read to an external store with selectors, and hot but narrow to colocated local state"
  caption="Channel choice is a frequency-and-fanout call: only hot + widely-read state earns a selector store; everything else is props, context, or local state."
  hue="var(--d-infra)"
/>
