...
import creepMechanisms from "./infographics/creep-mechanisms.svg?raw";
...
<Infographic
  svg={creepMechanisms}
  label="A taxonomy tree: the root 'creep-class regressions' branches into three mechanisms — bundle creep (+70 kB minified from the whole CommonJS lodash, detected by a hard-fail CI byte budget, fixed by a per-method import), hydration growth (+90 kB from a use-client cascade, detected by a client-bundle budget, fixed by isolating a tiny client leaf), and memo decay (4 ms then 41 ms per commit, detected by the Profiler plus RUM INP, fixed by a stable reference or the React Compiler)."
  caption="Three mechanisms, one failure class: each carries its own signature number, its own detector, and its own fix — and memo decay ships zero new bytes, which is why a byte budget alone never catches it."
  hue="var(--d-frontend)"
/>
