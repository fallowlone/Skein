...
<Infographic
  svg={detectionColumn}
  label="Taxonomy of the three React incidents, each branched into its detection signal, root cause, and prevention mechanism: render loop detected by a 12× RUM long-task spike with no exception; hydration storm detected by 40,000 errors per minute through onRecoverableError; subscription leak detected only by support tickets tracking session age."
  caption="Read down the detection column: not one incident began with an exception, and every prevention item is a mechanism — an alert, a gating metric, a synthetic — never an intention."
  hue="var(--d-infra)"
/>
