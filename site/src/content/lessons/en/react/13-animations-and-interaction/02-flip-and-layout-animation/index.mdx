...
import layoutCost from "./infographics/layout-cost.svg?raw";
...
<Infographic
  svg={layoutCost}
  label="Side-by-side cost comparison for a 200-card reorder at 60 Hz against a 16.7 ms frame budget. The naive top tween pays style recalc 1.5 ms plus layout 8 ms plus paint 4 ms — about 13.5 ms — on every frame and runs at 22 fps. FLIP pays two layout passes (Last forces ~8 ms once) and 200 el.animate calls once, then transform-only frames at a flat 60 fps."
  caption="The naive tween pays the full style-layout-paint pipeline every frame; FLIP pays layout only at the endpoints, then every animated frame is transform-only compositor work. Two layout passes per change versus one per frame is the whole technique."
  hue="var(--d-frontend)"
/>
