...
import themingRuntime from "./infographics/theming-runtime.svg?raw";
...
<Infographic
  svg={themingRuntime}
  label="Side-by-side comparison of two theming runtimes. CSS custom properties: zero re-render on switch (flip data-theme, the browser recomputes styles), SSR-safe and cross-framework, at the cost of untyped string values. JS theme objects: typed and composable values via context, at the cost of re-rendering every consumer (~1,800 components on a switch) and being framework-locked."
  caption="The runtime cost is the deciding axis: flipping data-theme re-styles through the cascade with zero React re-renders, while a context-held theme object must re-render every consumer to deliver new values."
  hue="var(--d-infra)"
/>
