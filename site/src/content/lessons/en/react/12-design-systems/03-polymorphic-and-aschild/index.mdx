...
<Infographic
  svg={asVsAschild}
  label="Side-by-side comparison of the two polymorphism mechanisms. Left, the as prop: runtime cost near zero (const Comp = as ?? div), but expensive in the type system — conditional and mapped types instantiated at every call site, scaling with usage (4,000 Box sites added about 30s of tsc time), plus a polymorphic ref per element; its home is style primitives like Box, Text, Stack. Right, asChild and Slot: the types stay simple because each side keeps its own props, and the cost moves to runtime — Slot clones the one child and merges prop sets (className concat, style child-wins, handlers child-first), throwing on multiple children and silently failing if the child drops props; its home is behavior wrappers like Button and every Trigger."
  caption="Same goal — one component, many elements — two opposite cost profiles: as pays in the type system (per-call-site instantiation), asChild pays in the element tree (Slot's merge mechanics and the child contract). Reserve as for style primitives, asChild for behavior wrappers."
  hue="var(--d-infra)"
/>
