...
<Infographic
  svg={memoDivision}
  label="Two-column comparison of the division of labor. The React Compiler covers per-expression caching, is finer-grained than useMemo, applies only where the Rules of React hold, and only to code in your pipeline. Manual memo still covers published library components, non-component caches and selectors, bridges over known bailouts, and referential identity for effects."
  caption="The compiler does not retire manual memo — it narrows it to the jobs it structurally cannot reach."
  hue="var(--d-frontend)"
/>
