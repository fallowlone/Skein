...
<FlowDiagram
  label="The audience confusion attack path: the authorization server mints a token for API-A, an attacker obtains it, presents it to API-B, whose weak aud check accepts any known client_id, resulting in a cross-audience breach."
  perRow={5}
  hue="var(--d-crypto)"
  nodes={[
    { id: "mint", label: "Token minted", sub: "aud: API-A" },
    { id: "steal", label: "Attacker holds it", sub: "leak / XSS" },
    { id: "present", label: "Sent to API-B", sub: "wrong audience" },
    { id: "weak", label: "Weak aud check", sub: "any client_id" },
    { id: "breach", label: "Cross-aud breach", sub: "acts as victim" },
  ]}
  edges={[
    { from: "mint", to: "steal" },
  ...
  caption="The hard-coded aud check breaks this chain at the fourth node: a token whose aud is API-A never matches API-B's expected identifier, so it is rejected before the breach."
/>
