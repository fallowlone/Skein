...
<Infographic
  svg={instanceScope}
  label="Side-by-side comparison: a per-request DataLoader gives a fresh cache, tenant isolation, and always-current rows; a module-scope global instance serves stale rows, leaks one tenant's cached row to another, and never reclaims its cache."
  caption="Same loader, opposite safety: per-request scope is correct; a global singleton is a shared cache that leaks tenant data and serves stale rows."
  hue="var(--d-infra)"
/>
