...
summary: "DataLoader waits for the current event-loop tick to finish, then fires one batched query for every key queued across all resolvers — turning N per-item SQL calls into one."
estMin: 13
status: ready
lessonType: topic
level: middle
...
spiral: ["statefulness"]
concepts: ["dataloader", "event-loop-tick", "request-scope-cache", "batch-dedup", "per-request-instance"]
sources:
