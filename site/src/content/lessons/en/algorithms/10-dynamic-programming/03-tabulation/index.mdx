...
summary: "Tabulation is DP bottom-up: build a dp array, fill base cases, then loop from small states up to the goal so every cell reads cells already filled. No recursion, no stack. Same O(n) time as memoization, and a rolling window can drop fib to O(1) space."
estMin: 25
status: ready
prereqs: ["02-memoization"]
mathPrereqs: []
concepts: ["tabulation", "bottom_up_dp", "dp_array", "base_case", "transition", "recurrence", "iteration_order", "rolling_array", "space_optimization", "fibonacci", "climbing_stairs"]
sources:
