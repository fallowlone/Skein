...
summary: "The 1D DP pattern: state is a single position i and dp[i] depends on a few earlier cells. House Robber is max(dp[i-1], dp[i-2]+nums[i]); Climbing Stairs is Fibonacci dp[i]=dp[i-1]+dp[i-2]; Decode Ways counts decodings with zero edge cases. O(n) time, O(1) space with rolling."
estMin: 28
status: ready
prereqs: ["03-tabulation"]
mathPrereqs: []
concepts: ["one_dimensional_dp", "house_robber", "climbing_stairs", "decode_ways", "dp_state", "recurrence", "base_case", "rolling_variables", "constant_space"]
sources:
