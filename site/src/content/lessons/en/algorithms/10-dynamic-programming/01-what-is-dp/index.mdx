...
<AlgoPoster
  ...
  id="ap-dp-what-is-dp"
  lang="en"
  kicker="same subproblem · solved again and again"
  lessonTag="lesson: algorithms / 10 / 01"
  cost="~2^n calls → n+1 distinct answers"
  title="NAIVE RECURSION REPEATS ITSELF: 9 CALLS, 5 ANSWERS"
  titleHi={["REPEATS ITSELF", "9 CALLS, 5 ANSWERS"]}
  sub="fib(4) makes 9 calls but asks only 5 distinct questions: fib(2) is solved twice, fib(1) three times, fib(0) twice. The tree doubles every level while the distinct answers stay flat at n+1."
  badges={["fib(n-1) + fib(n-2)", "overlap", "n+1 distinct"]}
  panelTitle="THE REPEAT MACHINE · ~9s / cycle"
  label="Animated recursion tree of fib(4) with nine nodes. The root splits, the left spine dives to the base cases, then the right branches light up as recomputed duplicates: fib(2) twice, fib(1) three times, fib(0) twice. Orange nodes are repeats the naive recursion solves from scratch."
  pattern="tree"
  data={{ title: "fib(4) naive recursion — 9 calls, 5 distinct", values: ["fib(4)", "fib(3)", "fib(2)", "fib(2)", "fib(1)", "fib(1)", "fib(0)", "fib(1)", "fib(0)"], visits: [[0], [0, 1, 3, 7], [0, 1, 2, 3, 4, 7], [0, 1, 2, 3, 4, 5, 6, 7, 8]], curs: [[0], [7], [2, 4], [2, 4, 5, 6, 8]], note: "orange = recomputed duplicates" }}
  steps={[
    { caption: "① fib(4) splits into fib(3) + fib(2)…", state: "STATE: root splits", log: "call fib(4) -> fib(3) + fib(2)" },
    { caption: "② left spine dives to the base cases…", state: "STATE: left spine done", log: "fib(3) -> fib(2) -> fib(1) base" },
    { caption: "③ right branch repeats fib(2) — solved again…", state: "STATE: fib(2) x2", log: "fib(2) again · recompute from scratch" },
    { caption: "④ 9 calls, only 5 distinct questions…", state: "STATE: 9 calls, 5 distinct", log: "fib(1) x3 · fib(0) x2 · 9 calls total" }
  ]}
  minis={[
    { title: "OVERLAP — the same call twice", body: "fib(2) appears in two branches; naive recursion solves each occurrence from scratch, doubling work every level.", tone: 1, at: 2 },
    { title: "THE DP FIX — solve once, reuse", body: "Only fib(0)..fib(4) are distinct: 5 answers. DP stores each one and reuses it, turning ~2^n calls into n+1.", tone: 0, at: 3 }
  ]}
  takeaway="takeaway: naive recursion recomputes the same subproblems — fib(2) twice, fib(1) three times — while only n+1 answers are distinct."
  src="src: lesson algorithms/10 · fib overlap"
/>
