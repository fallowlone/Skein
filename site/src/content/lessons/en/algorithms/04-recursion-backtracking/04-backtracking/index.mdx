...
<AlgoPoster
  ...
  id="ap-backtracking"
  lang="en"
  kicker="choose · explore · undo · prune"
  lessonTag="lesson: algorithms / 04 / 04"
  cost="pruned tree beats brute force"
  title="CHOOSE DEEP, HIT A WALL, UNDO, TRY NEXT"
  titleHi={["UNDO", "TRY NEXT"]}
  sub="From the empty partial solution pick a or b, extend once more: a,a violates the constraint and its subtree is pruned, so back up and try a,b — a valid leaf. Same on the right: b,a works, b,b is cut."
  badges={["choose-explore-undo", "is_valid() prunes", "DFS of partials"]}
  panelTitle="PRUNE AND BACK UP · ~9s / cycle"
  label="Animated decision tree: root branches a and b, each extends twice. The visit order walks a then a,a and stops at the dead end, backs up to a,b, then crosses to b,a and stops at b,b — two solution leaves, two pruned branches."
  pattern="tree"
  data={{ title: "decision tree: 2 solutions, 2 pruned", values: ["[ ]", "a", "b", "a,a", "a,b", "b,a", "b,b"], visits: [[0], [0, 1, 3], [0, 1, 3, 4], [0, 1, 2, 3, 4, 5, 6]], curs: [[0], [3], [4], [5]], note: "dead ends stop: whole subtree skipped" }}
  steps={[
    { caption: "① start empty, choose branch a…", state: "STATE: partial [a]", log: "choose a · recurse deeper" },
    { caption: "② extend to a,a — constraint broken, PRUNE…", state: "STATE: a,a dead ✗", log: "a,a violates · prune subtree" },
    { caption: "③ undo, try a,b — valid leaf, solution…", state: "STATE: a,b valid ✓", log: "undo a · choose b · record ✓" },
    { caption: "④ right side: b,a works, b,b pruned — done…", state: "STATE: 2 sols · 2 cuts", log: "b,a ✓ · b,b ✗ prune · done" }
  ]}
  minis={[
    { title: "UNDO IS NOT OPTIONAL", body: "Without erasing the choice, state leaks into the next branch and corrupts every later candidate. Choose, explore, undo — always all three.", tone: 2, at: 2 },
    { title: "PRUNE THE MOMENT IT BREAKS", body: "Check is_valid() before recursing. A doomed partial can never grow a valid leaf, so skip its whole subtree — that skip is the entire win.", tone: 1, at: 3 }
  ]}
  takeaway="takeaway: backtracking is DFS over partial solutions — undo keeps branches honest, pruning cuts branches that cannot win."
  src="src: lesson algorithms/04 · choose, prune, undo"
/>
