...
<StackDiagram
  label="The call stack at the deepest point of factorial(4): four frames stacked. factorial(4) is at the bottom (first call), factorial(1) — the base case — is on top (deepest call, runs first). Each recursive call pushes a frame up; each return pops the top frame off, so the stack unwinds factorial(1) → factorial(2) → factorial(3) → factorial(4)."
  hue="var(--d-systems)"
  layers={[
    { label: "factorial(1)  (top — base case)", note: "n=1 · pushed last, returns 1, pops first" },
    { label: "factorial(2)", note: "n=2 · waits, then 2 × 1 = 2" },
    { label: "factorial(3)", note: "n=3 · waits, then 3 × 2 = 6" },
    { label: "factorial(4)  (bottom — first call)", note: "n=4 · waits, then 4 × 6 = 24" },
  ]}
  caption="Maximum stack depth for factorial(4): the deepest call (factorial(1), the base case) sits on top; the first call (factorial(4)) sits at the bottom. Each call PUSHES a frame up with its own n; the base case stops the growth, then frames POP as each returns — factorial(1) → factorial(2) → factorial(3) → factorial(4), last in first out. Stack depth = recursion depth = O(n) space."
/>
