...
- Day 2: 8 + 9 = 17. Fits 10 (17 + 10 = 27 ≤ 32).
...
<AlgoPoster
  ...
  id="ap-binary-answer"
  lang="en"
  kicker="no array · search the answer space"
  lessonTag="lesson: algorithms / 03 / 06"
  cost="O(n · range) brute → O(n log range)"
  title="FEASIBLE STAYS RIGHT: HALVE TO THE BOUNDARY"
  titleHi={["HALVE", "BOUNDARY"]}
  sub="Ship capacity for weights 1..10 in 5 days lives in [10, 55]: below 15 infeasible, at 15 and above feasible. Each mid asks canShipInDays and keeps the half holding the flip — landing on 15."
  badges={["feasible(x) monotone", "hi = mid on yes", "lo = mid+1 on no"]}
  panelTitle="BRACKET THE FLIP, THEN HALVE · ~9s / cycle"
  label="Animated candidate strip 10, 12, 14, 15, 21, 32, 55 with answer 15 starred. Feasible mids 32, 21, 15 pull hi left while infeasible mids 12, 14 push lo right, until the bracket lands on [15,15]."
  pattern="halving"
  data={{ title: "capacity flip at 15", cells: ["10", "12", "14", "15", "21", "32", "55"], ranges: [[0, 6], [0, 5], [0, 3], [3, 3]], target: 3 }}
  steps={[
    { caption: "① [10,55]: mid 32 ships in 2 days — feasible, discard above…", state: "STATE: feasible hi 32", log: "mid=32 · days 2 ≤ 5 · hi=32" },
    { caption: "② [10,32]: mid 21 ships in 3 days — feasible, hi drops…", state: "STATE: feasible hi 21", log: "mid=21 · days 3 ≤ 5 · hi=21" },
    { caption: "③ [10,21]: mid 15 ships in 5 days, mid 12 needs 6 — bracket [13,15]…", state: "STATE: lo 13 hi 15", log: "mid=15 ok hi=15 · mid=12 no lo=13" },
    { caption: "④ mid 14 needs 6 days — lo meets hi at 15, the smallest feasible…", state: "STATE: ANSWER 15", log: "✓ mid=14 needs 6 · lo=15=hi · answer 15" }
  ]}
  minis={[
    { title: "MONOTONE OR NOTHING — false…false true…true", body: "Once feasible(x) flips to true it stays true. Without that, halves carry no information and binary search lies.", tone: 0, at: 0 },
    { title: "RANGE IS lo < hi — pin the boundary", body: "Feasible mid keeps hi = mid (a smaller answer may work); infeasible mid sets lo = mid + 1. They meet exactly on the flip.", tone: 1, at: 3 }
  ]}
  takeaway="takeaway: binary search on the answer brackets a monotone feasible/infeasible flip — O(log range) feasibility checks find the smallest working value."
  src="src: lesson algorithms/03 · binary search on answer"
/>
