...
<AlgoPoster
  ...
  id="ap-quicksort"
  lang="en"
  kicker="partition · pivot lands in its final slot"
  lessonTag="lesson: algorithms / 03 / 04"
  cost="O(n log n) average → O(n²) bad pivot"
  title="PARTITION AROUND THE PIVOT, THEN RECURSE"
  titleHi={["PARTITION", "RECURSE"]}
  sub="On [3, 7, 2, 9, 5, 1, 6] one O(n) partition walks pivot 5 into its final slot: smaller values left, larger right. Quicksort then recurses on [3, 2, 1] and [7, 9, 6] independently."
  badges={["pivot goes final", "recurse both sides", "in-place O(log n) avg"]}
  panelTitle="ONE PARTITION, TWO SUBPROBLEMS · ~9s / cycle"
  label="Animated bars for 3, 7, 2, 9, 5, 1, 6 with pivot 5 glowing. Smaller bars glide left of the pivot and larger ones right until 5 sits in its final middle slot, then the left and right regions sort independently into 1, 2, 3, 5, 6, 7, 9."
  pattern="bars"
  data={{ title: "pivot 5 finds its final slot", values: [3, 7, 2, 9, 5, 1, 6], orders: [[0, 1, 2, 3, 4, 5, 6], [0, 2, 5, 1, 3, 4, 6], [0, 2, 5, 4, 1, 3, 6], [5, 2, 0, 4, 6, 1, 3]], hi: [[4], [0, 2], [4], []], done: [0, 0, 1, 7] }}
  steps={[
    { caption: "① pick pivot 5 — everything sorts around it…", state: "STATE: pivot 5", log: "pivot=5 · partition [3,7,2,9,5,1,6]" },
    { caption: "② smaller values glide left, larger stay right…", state: "STATE: partitioning", log: "swap 7↔2 · 3,2,1 left" },
    { caption: "③ pivot 5 lands in its final slot — never moves again…", state: "STATE: pivot final", log: "✓ pivot 5 final · left [3,2,1] right [7,9,6]" },
    { caption: "④ recurse both sides: balanced halves give O(n log n)…", state: "STATE: SORTED", log: "✓ sorted 1,2,3,5,6,7,9 · log n levels" }
  ]}
  minis={[
    { title: "BALANCED IS FAST — log n levels", body: "A pivot near the middle halves the work each level: log n levels × O(n) = O(n log n) average.", tone: 0, at: 0 },
    { title: "EXTREME PIVOT IS O(n²) — fix it", body: "Always-min-or-max pivot gives n levels of O(n): sorted input plus naive pivot is the classic trap. Use random or median-of-three.", tone: 1, at: 3 }
  ]}
  takeaway="takeaway: partition parks the pivot in its final slot and splits the problem in two — balanced splits give O(n log n), extreme pivots O(n squared)."
  src="src: lesson algorithms/03 · quicksort"
/>
