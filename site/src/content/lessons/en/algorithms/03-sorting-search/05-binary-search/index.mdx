...
<AlgoPoster
  ...
  id="ap-binary-search"
  lang="en"
  kicker="sorted only · halve the range every step"
  lessonTag="lesson: algorithms / 03 / 05"
  cost="O(n) scan → O(log n) probes"
  title="CHECK MID, THROW AWAY HALF, REPEAT"
  titleHi={["THROW AWAY HALF", "REPEAT"]}
  sub="On sorted [1, 3, 5, 7, 9, 11, 13] hunting 13: mid 7 < 13 discards the left half, mid 11 < 13 discards again, mid 13 hits. Seven candidates collapse to one in 3 probes."
  badges={["lo..hi bounds", "mid from lo,hi", "O(log n) probes"]}
  panelTitle="THE RANGE HALVES EVERY STEP · ~9s / cycle"
  label="Animated sorted strip of 1, 3, 5, 7, 9, 11, 13 with target 13 starred. A bracket shows the live range shrinking 7 to 3 to 1 while the mid marker hops right twice and lands on 13."
  pattern="halving"
  data={{ title: "target 13 in sorted 7", cells: ["1", "3", "5", "7", "9", "11", "13"], ranges: [[0, 6], [4, 6], [6, 6], [6, 6]], target: 6 }}
  steps={[
    { caption: "① lo 0 hi 6: mid 3 holds 7 < 13, go right…", state: "STATE: range 7", log: "mid=3 val 7 < 13 · lo=4" },
    { caption: "② lo 4 hi 6: mid 5 holds 11 < 13, go right…", state: "STATE: range 3", log: "mid=5 val 11 < 13 · lo=6" },
    { caption: "③ lo 6 hi 6: mid 6 holds 13 = target, found…", state: "STATE: FOUND idx 6", log: "✓ mid=6 val 13 = target · return 6" },
    { caption: "④ 3 probes for 7 elements — 20 probes per million…", state: "STATE: O(log n)", log: "✓ 7→3→1 · O(log n)" }
  ]}
  minis={[
    { title: "SORTED IS THE CONTRACT — no order, no search", body: "Every discard assumes arr[mid] < target means right. On unsorted data that inference is false and the answer is wrong.", tone: 0, at: 0 },
    { title: "SHRINK OR LOOP FOREVER — mid plus/minus 1", body: "lo = mid + 1 and hi = mid − 1 drop mid itself. Reuse mid and the range never shrinks: infinite loop.", tone: 1, at: 3 }
  ]}
  takeaway="takeaway: binary search keeps [lo, hi], checks mid, and discards the half that cannot hold the target — O(log n) on sorted data."
  src="src: lesson algorithms/03 · binary search"
/>
