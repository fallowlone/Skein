...
summary: "A stack kept in sorted order (increasing or decreasing) by removing elements that violate the invariant before pushing. The canonical application: find the next greater element to the right of each item in O(n) time, not O(n²)."
estMin: 25
status: ready
prereqs: ["01-the-linked-list", "02-pointer-manipulation", "04-the-stack"]
mathPrereqs: []
concepts: ["monotonic-stack", "monotonic-decreasing", "invariant", "next-greater-element", "amortized", "push-pop"]
sources:
