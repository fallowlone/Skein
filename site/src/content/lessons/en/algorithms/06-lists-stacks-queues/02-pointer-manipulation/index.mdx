...
<AlgoPoster
  ...
  lang="en"
  kicker="save next · rewire · advance"
  lessonTag="lesson: algorithms / 06 / 02"
  cost="reverse in O(n) time · O(1) space"
  title="THE THREE-POINTER DANCE: SAVE, REWIRE, ADVANCE"
  titleHi={["SAVE", "REWIRE", "ADVANCE"]}
  sub="Reversing A → B → C with prev, cur and nxt. Overwriting cur.next before saving it loses the rest of the list — so each step saves nxt first, flips one link backward, then slides both pointers forward."
  badges={["nxt saved first", "one link flipped", "prev and cur slide"]}
  panelTitle="REVERSAL AS A CLOSED LOOP · ~9s / cycle"
  label="Animated chain of three nodes A, B, C. Pointer tags prev, cur and nxt glide along the chain: cur starts on A, each step flips one arrow backward and both pointers slide forward, until prev sits on C — the new head — and cur falls off the end."
  pattern="graph"
  data={{ title: "reverse A → B → C", nodes: ["A", "B", "C"], edges: [[0, 1], [1, 2]], waves: [[0], [1], [2], [2]], dones: [[], [0], [0, 1], [0, 1, 2]], dists: [["cur", "nxt", null], ["prev", "cur", "nxt"], ["", "prev", "cur"], ["", "", "prev"]], layout: "chain", note: "lose nxt and you lose the list" }}
  steps={[
    { caption: "① cur=A, prev=null: save nxt=B — the road to the rest…", state: "STATE: cur=A nxt=B", log: "save nxt=B · cur=A prev=null" },
    { caption: "② flip A.next backward to null, slide: prev=A, cur=B…", state: "STATE: prev=A cur=B", log: "A.next=null · prev=A cur=B" },
    { caption: "③ flip B.next backward to A, slide: prev=B, cur=C…", state: "STATE: prev=B cur=C", log: "B.next=A · prev=B cur=C" },
    { caption: "④ flip C.next backward to B, cur=null: prev=C is the new head…", state: "STATE: head=C done", log: "C.next=B · cur=null · head=C ✓" }
  ]}
  minis={[
    { title: "SAVE BEFORE YOU CUT — the golden rule", body: "cur.next is the only road to the rest of the list. Copy it to nxt first, overwrite second.", tone: 1, at: 0 },
    { title: "DUMMY HEAD — no special cases", body: "Anchor a dummy before the real head and every insert or delete has a prev — head ops use the same code.", tone: 3, at: 3 }
  ]}
  takeaway="takeaway: reversal flips one link per step with three pointers — save nxt, point cur back at prev, slide forward — O(n) time, O(1) space."
  src="src: lesson algorithms/06 · splice, unlink, reverse"
/>
</Idea>
