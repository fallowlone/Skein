...
<AlgoPoster
  client:visible
  id="ap-counting-steps"
  lang="en"
  kicker="count the compares · watch n grow"
  lessonTag="lesson: algorithms / 01 / 03"
  cost="1 loop → n · nested → n*n"
  title="EVERY ELEMENT PAYS ONE COMPARE: COUNT THE STEPS"
  titleHi={["ONE COMPARE", "COUNT THE STEPS"]}
  sub="largestOf on [3, 9, 2, 7] compares 9 vs 3, then 2, then 7 — three compares for four items. The bracket widens one cell per step; the sum chip counts the cost."
  badges={["1 element = 1 compare", "n items → n-1 steps", "nested → n*n"]}
  panelTitle="THE COUNT AS A SLIDING WINDOW · ~9s / cycle"
  label="Animated lane of four cells holding 3, 9, 2, 7. A dashed bracket starts on the first cell and widens rightward one cell per step while a counter chip tallies compares: 1, then 2, then 3."
  pattern="lane"
  data={{ title: "largestOf cost on 4 items", cells: ["3", "9", "2", "7"], wins: [[0, 0], [0, 1], [0, 2], [0, 3]], sums: ["compares=0 · start", "compares=1 · 9 wins", "compares=2 · 2 skipped", "compares=3 · done · n-1"], lo: "i", hi: "scan", note: "one new cell, one new compare — cost grows with n" }}
  steps={[
    { caption: "① Start: largest=3, zero compares so far…", state: "STATE: compares=0", log: "largest=3 · compares=0" },
    { caption: "② Cell 9: one compare, largest becomes 9…", state: "STATE: compares=1", log: "9>3 · largest=9 · compares=1" },
    { caption: "③ Cell 2: one more compare, 9 survives…", state: "STATE: compares=2", log: "2<9 · skip · compares=2" },
    { caption: "④ Cell 7: last compare — 3 compares for n=4.", state: "STATE: compares=3", log: "7<9 · skip · compares=3 · n-1" }
  ]}
  minis={[
    { title: "LINEAR — double n, double cost", body: "Single loop: 10 items cost 9, 100 cost 99. Growth follows n like a shadow.", tone: 0, at: 0 },
    { title: "NESTED — double n, quadruple cost", body: "Every pair checked: 10 items cost 45, 100 cost 4950. The n*n term pulls away fast.", tone: 3, at: 3 }
  ]}
  takeaway="takeaway: count the basic ops as a function of n — single loop grows with n, nested loops with n times n."
  src="src: lesson algorithms/01 · step counts n vs n*n"
/>
