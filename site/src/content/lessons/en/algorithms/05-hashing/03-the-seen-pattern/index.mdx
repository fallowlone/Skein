...
<AlgoPoster
  ...
  id="ap-seen"
  lang="en"
  kicker="store as you go · check as you go · O(n)"
  lessonTag="lesson: algorithms / 05 / 03"
  cost="O(n) time · O(n) space"
  title="REMEMBER WHAT YOU SAW: 9 FINDS 5 IN THE SET"
  titleHi={["REMEMBER", "9 FINDS 5"]}
  sub="Scan 5, 2, 11, 9 with target 14. For each x ask: is 14 − x in the seen set? Misses on 5, 2, 11 add them; on 9 the complement 5 is already there — an O(1) hit, answer 5 + 9 = 14."
  badges={["seen.has(comp)", "comp = t - x", "O(1) lookup"]}
  panelTitle="THE SEEN SET GROWS · ~9s / cycle"
  label="Animated set table with four buckets. Elements 5, 2, 11 drop into the set one per step while their complements miss; then 9 arrives and points at the stored 5 — a lookup hit that ends the scan."
  pattern="buckets"
  data={{ title: "target 14: complements 9, 12, 3, 5", nb: 4, keys: ["5", "2", "11", "9"], to: [1, 2, 3, 0], placed: [1, 2, 3, 4], hi: [0, 1, 2, 3], hlabels: ["h=1", "h=2", "h=3", "h=0"], note: "9 asks: is 5 in the set? HIT" }}
  steps={[
    { caption: "① x=5: complement 9 — set empty, miss, store 5…", state: "STATE: seen {5}", log: "x=5 · comp 9 · miss · add 5" },
    { caption: "② x=2: complement 12 — not there, store 2…", state: "STATE: seen {5,2}", log: "x=2 · comp 12 · miss · add 2" },
    { caption: "③ x=11: complement 3 — miss, store 11…", state: "STATE: seen {5,2,11}", log: "x=11 · comp 3 · miss · add 11" },
    { caption: "④ x=9: complement 5 IS in the set — HIT, done…", state: "STATE: 5+9=14 ✓", log: "x=9 · comp 5 · HIT ✓ 5+9=14" }
  ]}
  minis={[
    { title: "STORE AS YOU GO, CHECK AS YOU GO", body: "Each element first asks about its complement, then joins the set. The set always holds exactly the elements seen so far — the past, queryable in O(1).", tone: 0, at: 0 },
    { title: "ONE SHAPE, FIVE PROBLEMS", body: "Two-sum, duplicates, intersection, first repeat, visited nodes — all ask: does something I saw match the current element? Spot it, reach for a set.", tone: 1, at: 3 }
  ]}
  takeaway="takeaway: the seen set turns the past into O(1) memory — one pass, no nested loop, O(n) time for O(n) space."
  src="src: lesson algorithms/05 · seen-set two-sum"
/>
