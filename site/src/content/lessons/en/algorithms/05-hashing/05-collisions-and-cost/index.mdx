...
summary: "A hash table achieves O(1) average lookup only when keys spread evenly across buckets. Collisions degrade performance, but chaining, load factor management, and rehashing keep average time O(1). Worst case is O(n) with a bad hash function."
estMin: 20
status: ready
