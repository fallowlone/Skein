...
<AlgoPoster
  ...
  id="ap-collisions"
  lang="en"
  kicker="collision → chain · α = items / buckets"
  lessonTag="lesson: algorithms / 05 / 05"
  cost="O(1) average · O(n) worst"
  title="SAME BUCKET? CHAIN UP — SHORT CHAINS STAY O(1)"
  titleHi={["CHAIN UP", "O(1)"]}
  sub="Alice and April both hash to bucket 0 — a collision. Chaining stores a linked list per bucket: bucket 0 holds Alice then April, bucket 1 holds Bob then Ben. Lookup hashes, then walks the short chain."
  badges={["bucket = chain", "α = n / buckets", "resize at 0.75"]}
  panelTitle="COLLIDE, CHAIN, RESIZE · ~9s / cycle"
  label="Animated table with five buckets. Alice and April land in bucket 0 and stack into a two-link chain, Bob and Ben chain in bucket 1 — then a lookup of April hashes to 0 and walks the chain link by link."
  pattern="buckets"
  data={{ title: "h=first % 5: 0,0,1,1", nb: 5, keys: ["Alice", "April", "Bob", "Ben"], to: [0, 0, 1, 1], placed: [1, 2, 4, 4], hi: [0, 1, 2, 1], hlabels: ["h=0", "h=0", "h=1", "h=1"], note: "bucket 0 = chain Alice → April" }}
  steps={[
    { caption: "① Alice hashes to 0: bucket empty, store…", state: "STATE: b0=[Alice]", log: "hash Alice % 5 = 0 · store" },
    { caption: "② April hashes to 0 too: COLLISION — chain it…", state: "STATE: b0 chain x2", log: "hash April % 5 = 0 · chain!" },
    { caption: "③ Bob and Ben hash to 1: second chain…", state: "STATE: b1 chain x2", log: "hash Bob,Ben % 5 = 1 · chain" },
    { caption: "④ lookup April: hash to 0, walk Alice → April…", state: "STATE: found O(1) ✓", log: "lookup April → b0 · walk 2 ✓" }
  ]}
  minis={[
    { title: "LOAD FACTOR RUNS THE SHOW", body: "α = items ÷ buckets is the average chain length, and lookup is O(1 + α). Past ~0.75 the table doubles and rehashes everything — rare O(n), amortized O(1).", tone: 0, at: 1 },
    { title: "WORST CASE IS ONE BUCKET", body: "A bad hash — or crafted attack keys — piles all n keys into one chain. Then lookup walks a linked list: O(n). Uniform hash plus low α is the whole guarantee.", tone: 2, at: 3 }
  ]}
  takeaway="takeaway: chaining survives collisions with per-bucket lists — keep α low by resizing and lookup stays O(1) average."
  src="src: lesson algorithms/05 · chains + load"
/>
