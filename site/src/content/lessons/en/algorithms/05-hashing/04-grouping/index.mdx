...
summary: "Partition a collection by computing a group key for each item. Store items in a list under their key: map[key].push(item). One O(n) pass groups everything; the heart is key design."
estMin: 18
status: ready
