...
<AlgoPoster
  ...
  id="ap-grouping"
  lang="en"
  kicker="key = f(item) · map[key].push(item)"
  lessonTag="lesson: algorithms / 05 / 04"
  cost="O(n x m log m) · O(n) space"
  title="SAME KEY, SAME BUCKET, SAME GROUP"
  titleHi={["SAME KEY", "SAME GROUP"]}
  sub="Sort each word into its canonical key: eat, tea, ate all become aet and pile into one bucket; tan, nat become ant and pile into another. The map holds key to list — grouping answers which ones, not how many."
  badges={["key = sort(word)", "map[key].push", "key → list"]}
  panelTitle="WORDS SORT INTO BUCKETS · ~9s / cycle"
  label="Animated table with three buckets. Five words arrive across four steps — eat, tea, ate stack into bucket 0 under key aet while tan, nat stack into bucket 2 under key ant — forming two anagram groups."
  pattern="buckets"
  data={{ title: "sort letters → aet x3, ant x2", nb: 3, keys: ["eat", "tea", "ate", "tan", "nat"], to: [0, 0, 0, 2, 2], placed: [1, 3, 4, 5], hi: [0, 2, 3, 4], hlabels: ["k=aet", "k=aet", "k=aet", "k=ant", "k=ant"], note: "same key → same bucket → same group" }}
  steps={[
    { caption: "① eat sorts to aet: new key, new list…", state: "STATE: aet=[eat]", log: "eat → key aet · new list" },
    { caption: "② tea and ate sort to aet too: same bucket…", state: "STATE: aet x3", log: "tea → aet · ate → aet · push" },
    { caption: "③ tan sorts to ant: second key, second list…", state: "STATE: ant=[tan]", log: "tan → key ant · new list" },
    { caption: "④ nat joins ant: two groups complete…", state: "STATE: 2 groups ✓", log: "nat → ant · 2 groups ✓" }
  ]}
  minis={[
    { title: "KEY DESIGN IS THE WHOLE GAME", body: "Sorted letters for anagrams, n % k for remainders, word[0] for initials. The key decides what same group means — get it right and grouping writes itself.", tone: 0, at: 0 },
    { title: "GROUPING vs FREQUENCY", body: "Frequency stores a count per key: how many? Grouping stores a list per key: which ones? Same one-pass shape, different value.", tone: 1, at: 3 }
  ]}
  takeaway="takeaway: grouping is key = f(item), then push — items that belong together collide on purpose under one key."
  src="src: lesson algorithms/05 · anagrams by key"
/>
