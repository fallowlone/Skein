...
summary: "A window [left, right] slides across an array. Instead of recomputing a property for every subarray (O(n²)), you slide and update in O(1) per step for O(n) total. Two shapes: fixed-size (length k) and variable-size (grow/shrink based on condition)."
estMin: 20
status: ready
prereqs: ["03-two-pointers"]
mathPrereqs: []
concepts: ["sliding_window", "window", "subarray", "constraint", "two_pointers"]
sources:
