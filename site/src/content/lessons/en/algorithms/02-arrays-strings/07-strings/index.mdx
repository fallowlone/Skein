...
<StructureFigure
  ...
  caption={`The string "HELLO" is an array of 5 characters in contiguous memory, indexed 0..4. s[0] is 'H', s[4] is 'O' — any character is one address calculation away, so access is O(1). The cells are read-only: you cannot overwrite s[1] in place. Changing a character means building a whole new 5-cell string, which is why a string is immutable.`}
/>
