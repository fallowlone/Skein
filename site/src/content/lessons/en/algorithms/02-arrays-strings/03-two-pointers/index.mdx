...
order: 3
title: "Two pointers"
summary: "Keep two indices into the array and move them under a rule. Transforms O(n²) nested-loop problems into O(n) single-pass solutions. Two shapes: opposite-ends (start at both ends, move toward each other) and same-direction (both start front, one runs ahead)."
estMin: 19
status: ready
prereqs: ["02-traversal"]
mathPrereqs: []
concepts: ["two_pointers", "opposite_ends", "fast_slow", "in_place"]
sources:
