...
summary: "Visit nodes level by level from top to bottom, left to right, using a queue. The breadth-first alternative to depth-first recursion. Why queues work for level-order, the basic loop, and the key variant: track queue size at level start to process nodes by depth."
estMin: 22
status: ready
prereqs: ["01-the-binary-tree", "02-traversals"]
mathPrereqs: []
concepts: ["level-order", "breadth-first", "BFS", "queue", "FIFO", "visit", "traversal"]
sources:
