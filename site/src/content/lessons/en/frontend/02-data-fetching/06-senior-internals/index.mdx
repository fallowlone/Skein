...
After this lesson you'll be able to explain the RSC Payload format, trace a multi-layer cache invalidation bug, and recognise a hydration mismatch before the user reports it.
...
Five layers means five failure modes. A mutation that invalidates layers 1–4 but misses layer 5 leaves the user staring at stale data for up to `gcTime` (5 minutes by default) — and there's no error, no warning, just wrong numbers on screen.
...
When you see a button that sometimes ignores a click — not throws, just ignores — suspect a hydration mismatch before you suspect the event handler logic.
...
<Infographic
  svg={serverWaterfall}
  label="Column chart of TTFB on the same RSC page that fetches 50 items: a sequential for-await loop yields 2500ms, while Promise.all yields 100ms — a 25x difference from the same code restructured to fetch in parallel."
  caption="Same page, same data: sequential await stacks 50 round-trips into a 2.5s server-side waterfall; Promise.all fetches them in parallel for 100ms TTFB. The waterfall is invisible in code review."
  hue="var(--d-infra)"
/>
