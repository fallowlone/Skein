...
import deliverySemantics from "./infographics/delivery-semantics.svg?raw";
...
<Infographic
  svg={deliverySemantics}
  label="A two-way comparison of the only honest delivery guarantees. at-most-once acks before processing: no duplicates but possible loss on a crash. at-least-once acks after processing: no loss but possible duplicates, so the consumer must be idempotent."
  caption="There is no third honest option: the guarantee is decided entirely by WHEN you ack. Ack early and you risk loss; ack late and you risk duplicates — which is why almost everyone picks at-least-once and makes the consumer idempotent."
  hue="var(--d-infra)"
/>
