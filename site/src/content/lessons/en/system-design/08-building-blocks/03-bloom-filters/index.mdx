...
The **cuckoo filter** is a newer alternative that supports deletion and can be more space-efficient at low false-positive rates, at the cost of a more complex insert that can fail when the structure is too full. Counting and cuckoo are the two variants to reach for when plain-bloom's no-delete limitation bites.
