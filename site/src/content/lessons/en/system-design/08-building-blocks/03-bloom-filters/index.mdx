...
<Infographic
  svg={fpRateTuning}
  label="A column chart of bits per element against the false-positive rate it buys: 5 bits per element gives roughly 10% false positives, 10 bits gives about 1%, and 15 bits gives about 0.1% — each extra ~5 bits cuts the rate by roughly an order of magnitude."
  caption="The whole tuning knob: spend more bits per element and the false-positive rate falls ~10x per +5 bits — ~10 bits/element buys the canonical ~1%."
  hue="var(--d-infra)"
/>
