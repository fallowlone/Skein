...
import leaseTimeoutTradeoff from "./infographics/lease-timeout-tradeoff.svg?raw";
...
<Infographic
  svg={leaseTimeoutTradeoff}
  label="Side-by-side comparison of a long lease timeout versus a short one. A long lease makes split-brain rarer but failover slow; a short lease makes failover fast but split-brain likelier; and both columns share the same bottom row — split-brain is still possible — because no timeout can tell a dead leader from a frozen one about to wake."
  caption="Lengthening or shortening the lease only trades failover speed against split-brain probability — neither removes it. That dead end is precisely why the real fix lives at the resource, as a fencing token."
  hue="var(--d-infra)"
/>
