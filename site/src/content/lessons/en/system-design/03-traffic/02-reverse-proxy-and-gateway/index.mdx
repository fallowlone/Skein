...
import concentrationCosts from "./infographics/concentration-costs.svg?raw";
...
<Infographic
  svg={concentrationCosts}
  label="A list of the four costs a gateway concentrates: single point of failure (one config error fails every service), throughput bottleneck (all traffic plus per-request work on one tier), latency tax (an extra hop and CPU on every call), and deployment chokepoint (the gateway team becomes the bottleneck for shipping)."
  caption="The flip side of the consistency win: one tier on the critical path of every request concentrates four distinct costs at once — which is why the gateway must run redundant and stay thin."
  hue="var(--d-infra)"
/>
