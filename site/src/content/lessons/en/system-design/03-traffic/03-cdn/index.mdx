...
<Infographic
  ...
  label="Column chart of the share of all requests that reach the origin at three cache hit ratios: 10% at a 90% hit ratio, 5% at 95%, and 1% at 99% — each rightward step roughly halving the previous column."
  caption="The miss fraction is what the origin actually serves, and it halves with each step: 90%→95% takes origin traffic from 10% to 5%, 95%→99% from 5% to 1%. That non-linearity is why a few points of hit ratio shrink the expensive origin fleet so much."
  hue="var(--d-infra)"
/>
