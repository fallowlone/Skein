...
import storageOver5yr from "./infographics/storage-over-5yr.svg?raw";
...
<Infographic
  svg={storageOver5yr}
  label="Column chart of projected storage over five years: text is about 0.5 PB while media is about 50 PB — the media column towers ~100x over text, both derived from the same napkin inputs."
  caption="Same inputs, same two minutes: the estimate surfaces that media (~50 PB) dwarfs text (~0.5 PB) by 100x — so the storage design must center on media, not tweets."
  hue="var(--d-infra)"
/>
