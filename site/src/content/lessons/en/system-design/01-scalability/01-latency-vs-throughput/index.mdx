...
import queueingKnee from "./infographics/queueing-knee.svg?raw";
...
<Infographic
  svg={queueingKnee}
  label="Column chart of the M/M/1 latency multiplier as utilization rises: at 50% utilization latency is 2x the unloaded service time, at 70% about 3.3x, at 90% 10x, and at 99% it jumps to 100x — a near-vertical final bar."
  caption="The queueing knee: latency = 1/(1−utilization) × unloaded time. It stays flat until the last stretch, then goes vertical — which is why mature systems run at 60–70%, not 95%."
  hue="var(--d-infra)"
/>
