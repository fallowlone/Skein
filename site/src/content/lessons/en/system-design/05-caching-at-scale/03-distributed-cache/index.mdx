...
<Infographic
  svg={remapFraction}
  label="Column chart comparing the share of keys remapped when scaling a cache from 4 to 5 nodes: modulo hash(key) % N remaps about 80 percent of keys, while consistent hashing moves only about 1/N — roughly 20 percent — of them."
  caption="For a cache a remap is a cold miss: modulo nukes ~80% of the cache on a single node add, consistent hashing bounds it to one arc (~1/N)."
  hue="var(--d-infra)"
/>
