...
import invalidationTradeoffs from "./infographics/invalidation-tradeoffs.svg?raw";
...
<Infographic
  svg={invalidationTradeoffs}
  label="A taxonomy tree of the three cache-invalidation approaches. TTL expiry: the write path does nothing and the cache self-heals, but a value can be stale up to the TTL. Explicit delete: deleting the key on every write drives staleness near zero, but the write path is coupled to every derived key, a missed site stays stale forever, and it is prone to the write-time delete race. Versioned keys: bumping the version per write is race-free because no shared key is mutated, at the cost of orphaned old versions to evict and a version source to track."
  caption="There is no free option: TTL trades freshness for simplicity, explicit delete trades coupling and a race for freshness, and versioning trades orphans for race-freedom — you pick where the bug lives."
  hue="var(--d-infra)"
/>
