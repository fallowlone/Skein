...
import evictionPolicyBets from "./infographics/eviction-policy-bets.svg?raw";
...
<Infographic
  svg={evictionPolicyBets}
  label="A tree rooted at 'which entry to sacrifice when full' branching to four eviction policies: LRU bets recency predicts reuse (the default), LFU bets popularity, FIFO ignores access and evicts the oldest inserted, and Random is cheap and scan-resistant."
  caption="Each eviction policy is a different bet about the future encoded as a rule about the past — choosing wrong silently evicts your most valuable data."
  hue="var(--d-infra)"
/>
