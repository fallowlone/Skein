...
<Infographic
  ...
  label="Column chart of how many boxes to run for a 4-unit peak when each box serves 1 unit: N needs 4 with no spare, N+1 needs 5 to survive one failure, N+2 needs 6 to survive a second failure while one is already down."
  caption="Redundancy is spare capacity sized against peak: N (4) tolerates zero failures, every '+1' is one extra box that buys one more tolerated failure."
  hue="var(--d-infra)"
/>
