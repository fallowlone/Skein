...
В режиме **awsvpc** — обязательном для Fargate, современном дефолте для EC2 — каждая задача получает **собственный сетевой интерфейс (ENI)** с приватным IP в твоей подсети. В этом и фича: задача — полноправный гражданин VPC со своей security group, своим IP и прямыми flow-логами, вместо того чтобы делить интерфейс хоста и жонглировать портами (режимы `bridge`/`host`). Цена в том, что ENI и IP — **реальные счётные ресурсы**, и оба можно исчерпать.
