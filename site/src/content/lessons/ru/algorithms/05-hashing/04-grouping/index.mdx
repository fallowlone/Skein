...
<AlgoPoster
  ...
  id="ap-grouping"
  lang="ru"
  kicker="ключ = f(item) · map[key].push(item)"
  lessonTag="урок: algorithms / 05 / 04"
  cost="O(n x m log m) · O(n) памяти"
  title="ОДИН КЛЮЧ — ОДНО ВЕДРО — ОДНА ГРУППА"
  titleHi={["ОДИН КЛЮЧ", "ОДНА ГРУППА"]}
  sub="Отсортируй каждое слово в канонический ключ: eat, tea, ate все становятся aet и складываются в одно ведро; tan, nat становятся ant и складываются в другое. Словарь держит ключ к списку — группировка отвечает какие, а не сколько."
  badges={["key = sort(word)", "map[key].push", "key → list"]}
  panelTitle="СЛОВА СОРТИРУЮТСЯ В ВЁДРА · ~9с / цикл"
  label="Анимированная таблица на три ведра. Пять слов прибывают за четыре шага — eat, tea, ate складываются в ведро 0 под ключом aet, а tan, nat складываются в ведро 2 под ключом ant — образуя две группы анаграмм."
  pattern="buckets"
  data={{ title: "сортировка букв → aet x3, ant x2", nb: 3, keys: ["eat", "tea", "ate", "tan", "nat"], to: [0, 0, 0, 2, 2], placed: [1, 3, 4, 5], hi: [0, 2, 3, 4], hlabels: ["k=aet", "k=aet", "k=aet", "k=ant", "k=ant"], note: "один ключ → одно ведро → одна группа" }}
  steps={[
    { caption: "① eat сортируется в aet: новый ключ, новый список…", state: "STATE: aet=[eat]", log: "eat → key aet · new list" },
    { caption: "② tea и ate тоже сортируются в aet: то же ведро…", state: "STATE: aet x3", log: "tea → aet · ate → aet · push" },
    { caption: "③ tan сортируется в ant: второй ключ, второй список…", state: "STATE: ant=[tan]", log: "tan → key ant · new list" },
    { caption: "④ nat вступает в ant: две группы готовы…", state: "STATE: 2 groups ✓", log: "nat → ant · 2 groups ✓" }
  ]}
  minis={[
    { title: "ДИЗАЙН КЛЮЧА — ЭТО ВСЯ ИГРА", body: "Отсортированные буквы для анаграмм, n % k для остатков, word[0] для инициалов. Ключ решает, что значит одна группа, — подбери верно и группировка напишется сама.", tone: 0, at: 0 },
    { title: "ГРУППИРОВКА vs ЧАСТОТА", body: "Частота хранит счёт на ключ: сколько? Группировка хранит список на ключ: какие? Та же форма за проход, другое значение.", tone: 1, at: 3 }
  ]}
  takeaway="вывод: группировка — это ключ = f(item), потом push: элементы, что должны быть вместе, нарочно коллизируют под одним ключом."
  src="src: урок algorithms/05 · анаграммы по ключу"
/>
