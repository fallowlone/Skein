...
summary: "Разбей коллекцию по вычисленному ключу группы для каждого предмета. Храни предметы в списке под их ключом: map[key].push(item). Один O(n) проход группирует всё; сердце — дизайн ключа."
estMin: 18
status: ready
prereqs: ["02-frequency-counting"]
mathPrereqs: []
concepts: ["grouping", "group_key", "bucket", "anagram"]
sources:
