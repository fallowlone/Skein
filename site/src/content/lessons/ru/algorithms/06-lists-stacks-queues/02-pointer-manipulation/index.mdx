...
<AlgoPoster
  client:visible
  id="ap-pointer-manipulation"
  lang="ru"
  kicker="сохрани next · перенаправь · продвинь"
  lessonTag="урок: algorithms / 06 / 02"
  cost="разворот за O(n) времени · O(1) памяти"
  title="ТАНЕЦ ТРЁХ УКАЗАТЕЛЕЙ: СОХРАНИ, ПЕРЕНАПРАВЬ, ПРОДВИНЬ"
  titleHi={["СОХРАНИ", "ПЕРЕНАПРАВЬ", "ПРОДВИНЬ"]}
  sub="Разворачиваем A → B → C с prev, cur и nxt. Перезапись cur.next до сохранения теряет остаток списка — поэтому каждый шаг сначала сохраняет nxt, переворачивает одну ссылку назад, потом сдвигает оба указателя вперёд."
  badges={["nxt сохраняется первым", "одна ссылка перевёрнута", "prev и cur скользят"]}
  panelTitle="РАЗВОРОТ КАК ЗАМКНУТЫЙ ЦИКЛ · ~9с / цикл"
  label="Анимированная цепочка из трёх узлов A, B, C. Метки указателей prev, cur и nxt скользят вдоль цепочки: cur стартует на A, каждый шаг переворачивает одну стрелку назад и оба указателя сдвигаются вперёд, пока prev не встанет на C — новую голову — а cur не упадёт с конца."
  pattern="graph"
  data={{ title: "разворот A → B → C", nodes: ["A", "B", "C"], edges: [[0, 1], [1, 2]], waves: [[0], [1], [2], [2]], dones: [[], [0], [0, 1], [0, 1, 2]], dists: [["cur", "nxt", null], ["prev", "cur", "nxt"], ["", "prev", "cur"], ["", "", "prev"]], layout: "chain", note: "потеряешь nxt — потеряешь список" }}
  steps={[
    { caption: "① cur=A, prev=null: сохраняем nxt=B — дорогу к остатку…", state: "STATE: cur=A nxt=B", log: "save nxt=B · cur=A prev=null" },
    { caption: "② переворачиваем A.next назад на null, сдвигаем: prev=A, cur=B…", state: "STATE: prev=A cur=B", log: "A.next=null · prev=A cur=B" },
    { caption: "③ переворачиваем B.next назад на A, сдвигаем: prev=B, cur=C…", state: "STATE: prev=B cur=C", log: "B.next=A · prev=B cur=C" },
    { caption: "④ переворачиваем C.next назад на B, cur=null: prev=C — новая голова…", state: "STATE: head=C done", log: "C.next=B · cur=null · head=C ✓" }
  ]}
  minis={[
    { title: "СНАЧАЛА СОХРАНИ, ПОТОМ РЕЖЬ — золотое правило", body: "cur.next — единственная дорога к остатку списка. Сначала скопируй его в nxt, потом перезаписывай.", tone: 1, at: 0 },
    { title: "DUMMY HEAD — без особых случаев", body: "Поставь dummy перед настоящей головой, и у каждой вставки или удаления будет prev — операции с головой идут тем же кодом.", tone: 3, at: 3 }
  ]}
  takeaway="вывод: разворот переворачивает по одной ссылке за шаг тремя указателями — сохрани nxt, укажи cur назад на prev, сдвинься вперёд — O(n) времени, O(1) памяти."
  src="src: урок algorithms/06 · splice, unlink, reverse"
/>
</Idea>
