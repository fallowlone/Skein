...
summary: "Окно [left, right] скользит по массиву. Вместо пересчёта свойства для каждого подмассива (O(n²)), ты скользишь и обновляешь за O(1) за шаг, итого O(n). Два варианта: окно фиксированного размера (длина k) и переменного размера (растёшь/сжимаешь по условию)."
estMin: 20
status: ready
