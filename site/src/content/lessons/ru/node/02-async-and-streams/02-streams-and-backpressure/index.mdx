...
<Infographic
  svg={streamTypes}
  label="Таксономия четырёх типов потоков Node от корня: Readable (источник, из которого тянут — чтение файла, тело HTTP-запроса, сокет), Writable (сток, в который пушат — запись файла, HTTP-ответ, сокет; write() возвращает false за highWaterMark), Duplex (обе стороны независимы — TCP-сокет) и Transform (Duplex, отображающий вход в выход — gzip, шифр, парсер CSV)."
  caption="Четыре типа потоков, одна модель: Readable тянет, Writable пушит (и сигналит backpressure возвратом write()), Duplex делает оба с независимыми буферами, а Transform — это Duplex, отображающий вход в выход, потоковый map."
  hue="var(--d-infra)"
/>
