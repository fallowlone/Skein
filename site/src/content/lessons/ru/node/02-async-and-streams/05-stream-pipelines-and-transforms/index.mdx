...
<Infographic
  svg={transformMemory}
  label="Сравнение двух стратегий авторинга Transform бок о бок. Пуш по чанку в _transform держит рабочую память на уровне ~3×16КБ на трёхэтапном конвейере, так что вход 4ГБ течёт примерно в 48КБ. Буферизация каждого чанка ради отдачи в _flush держит весь датасет в RAM, так что вход 4ГБ стоит 4ГБ и роняет процесс по OOM."
  caption="Та же работа, два профиля памяти: пуш-по-ходу держится плоско на ~48КБ вдоль конвейера; буферизация-всего-в-_flush растёт с размером входа, и поток 4ГБ стоит 4ГБ, а затем OOM."
  hue="var(--d-infra)"
/>
