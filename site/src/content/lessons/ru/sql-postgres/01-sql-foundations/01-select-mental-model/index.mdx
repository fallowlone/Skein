...
<Infographic
  svg={aliasVisibility}
  label="Сравнение клауз SQL в двух колонках по тому, когда они бегут относительно SELECT: WHERE (шаг 2), GROUP BY (шаг 3) и HAVING (шаг 4) выполняются до SELECT, поэтому алиас SELECT ещё не создан; SELECT (шаг 5) и ORDER BY (шаг 7) бегут на нём или после, поэтому алиас доступен."
  caption="Алиас рождается в SELECT (шаг 5): клаузы до него назвать его не могут (WHERE падает), клаузы на нём и после — могут (ORDER BY разрешается) — всё противоречие «здесь работает, там ошибка» в одном правиле."
  hue="var(--d-data)"
/>
