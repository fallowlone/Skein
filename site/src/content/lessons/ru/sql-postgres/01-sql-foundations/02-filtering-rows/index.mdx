...
import sargableLatency from "./infographics/sargable-latency.svg?raw";
...
<Infographic
  svg={sargableLatency}
  label="Столбчатая диаграмма задержки запроса на таблице users в 20 миллионов строк: sargable-предикат email = $1 выполняется как index scan за 2 мс, а не-sargable lower(email) = $1 — как sequential scan за 1.9 секунды."
  caption="Та же таблица, тот же индекс, те же данные — изменилась лишь форма предиката. Голая колонка — index scan за 2 мс; обёртка в lower() — полный скан за 1.9 с, регрессия ~900× из урока."
  hue="var(--d-data)"
/>
