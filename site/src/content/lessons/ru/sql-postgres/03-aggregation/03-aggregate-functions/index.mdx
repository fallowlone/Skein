...
import filterOnePass from "./infographics/filter-one-pass.svg?raw";
...
<Infographic
  svg={filterOnePass}
  label="Столбчатая диаграмма сравнения времени на таблице orders в 12М строк: один запрос FILTER, читающий каждую строку один раз, занимает около 850мс, тогда как три отдельных запроса WHERE, сканирующих таблицу трижды, занимают около 2400мс."
  caption="Те же три счёта, два плана: FILTER маршрутизирует каждую строку в подходящие агрегаты за один проход (~850мс) против трёх последовательных сканов всей таблицы (~2.4с)."
  hue="var(--d-infra)"
/>
