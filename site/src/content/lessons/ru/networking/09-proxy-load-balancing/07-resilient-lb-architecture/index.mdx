...
- statefulness
status: ready
summary: Единственный LB — это SPOF — anycast + BGP ECMP устраняет его; zone-aware
  маршрутизация снижает стоимость межзонального трафика; TLS завершается на edge;
  метрики RED и состояние circuit breaker — минимально необходимый observability для
...
<FlowDiagram
  label="Anycast плюс BGP ECMP устраняет единственную точку отказа балансировщика: три машины LB все анонсируют один anycast VIP, и ECMP-маршрутизация сети хеширует 5-tuple каждого клиентского потока на одну из них. Если один LB падает, BGP отзывает его маршрут, и ECMP перехеширует его потоки на выжившие менее чем за секунду."
  hue="var(--d-network)"
  perRow="3"
  nodes={[
    { id: "vip", label: "Anycast VIP", sub: "203.0.113.10", col: 1, row: 0 },
    { id: "lba", label: "LB_A", sub: "анонсирует VIP", col: 0, row: 1 },
    { id: "lbb", label: "LB_B", sub: "анонсирует VIP", col: 1, row: 1 },
    { id: "lbc", label: "LB_C", sub: "анонсирует VIP", col: 2, row: 1 },
  ]}
  edges={[
    { from: "vip", to: "lba", label: "ECMP хеш 5-tuple" },
    { from: "vip", to: "lbb", label: "ECMP" },
    { from: "vip", to: "lbc", label: "ECMP" },
  ]}
  caption="Один VIP, много stateless LB. ECMP распределяет потоки по ним по хешу 5-tuple; маршрут упавшего LB отзывается BGP, и его потоки перехешируются на LB_B или LB_C менее чем за секунду — в кластере нет машины, потеря которой выводит трафик из строя."
/>
