...
summary: "Каждое поле в заголовках IPv4 и IPv6 имеет назначение: TTL предотвращает петли, DSCP маркирует приоритет, поле протокола передаёт payload нужному обработчику L4."
estMin: 12
status: ready
lessonType: topic
level: middle
prereqs: ["01-the-ip-envelope"]
deepensInto: ["03-routing-and-forwarding"]
spiral: ["encapsulation"]
concepts: ["ipv4-header", "ttl", "dscp", "ecn", "protocol-field", "checksum", "ipv6-header", "next-header"]
sources:
