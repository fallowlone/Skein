...
<Infographic
  svg={headerTradeoff}
  label="Сравнение заголовков IPv4 и IPv6 бок о бок. IPv4: минимум 20 байт, контрольная сумма пересчитывается на каждом хопе, роутеры могут фрагментировать в пути, опции встроены в заголовок, TTL ограничивает хопы и Protocol выбирает обработчик L4. IPv6: фиксированные 40 байт, контрольной суммы нет вовсе, роутеры никогда не фрагментируют, опции цепляются через расширенные заголовки Next Header, а Hop Limit и Next Header повторяют роли TTL и Protocol плюс Flow Label."
  caption="IPv6 покупает меньший бюджет работы на каждом хопе — без пересчёта контрольной суммы, без фрагментации роутерами — ценой большего фиксированного заголовка и выноса опций в цепочку расширенных заголовков."
  hue="var(--d-infra)"
/>
