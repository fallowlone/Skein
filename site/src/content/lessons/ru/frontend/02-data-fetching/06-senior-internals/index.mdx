...
import serverWaterfall from "./infographics/server-waterfall.svg?raw";
...
<Infographic
  svg={serverWaterfall}
  label="Столбчатая диаграмма TTFB на одной RSC-странице, загружающей 50 элементов: последовательный цикл for-await даёт 2500ms, а Promise.all — 100ms; разница в 25 раз достигается тем же кодом, переписанным на параллельную загрузку."
  caption="Та же страница, те же данные: последовательный await складывает 50 round-trip'ов в 2.5s серверного водопада; Promise.all загружает их параллельно за 100ms TTFB. Водопад невидим на code review."
  hue="var(--d-infra)"
/>
