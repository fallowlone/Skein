...
describe("scorePlacement", () => {
  it("knowing nothing yields a low estimate and the A1 starting band", () => {
    const r = scorePlacement(items, new Set());
    expect(r.estimatedKnown).toBeLessThan(400);
    expect(r.band).toBe("A1");
  });

  it("knowing every real word (no false alarms) maxes the estimate and starts at B1", () => {
  ...
  it("mastering only A1 starts the learner at A2", () => {
    const yes = idxYes((i) => !i.isPseudo && i.band === "A1");
    const r = scorePlacement(items, yes);
