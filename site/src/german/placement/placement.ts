// site/src/german/placement/placement.ts
//
// Vocab-size placement for the German layer. A yes/no recognition checklist
// sampled across frequency bands, with pseudoword controls. The false-alarm rate
// on pseudowords corrects over-claiming (standard yes/no correction
// h* = (h - f) / (1 - f)). Output: estimated known word-family count + the
// starting band. Pure; RNG injected. Mirrors site/src/english/placement/placement.ts.
...
export type PlacementScore = {
  ...
  band: GerBand;         // starting band for new words
...
export function buildPlacement(rng: () => number): PlacementItem[] {
  ...
  const fake: PlacementItem[] = PSEUDOWORDS.map((lemma) => ({
    lemma, rank: 0, band: "B1" as GerBand, isPseudo: true,
  }));
...
export function scorePlacement(items: PlacementItem[], yes: Set<number>): PlacementScore {
  ...
  const perBand: Record<GerBand, number> = { A1: 0, A2: 0, B1: 0 };
  ...
  let band: GerBand = "A1";
  if (perBand.A1 >= MASTERY) band = "A2";
  if (perBand.A1 >= MASTERY && perBand.A2 >= MASTERY) band = "B1";
