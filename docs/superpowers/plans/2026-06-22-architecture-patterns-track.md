...
- [ ] **P6 — Hazard scan (controller).** Grep all new files for `system-reminder`, `antml:`, `<function`; grep MDX for a backslash-quote inside `"..."` attributes and for unclosed `<Inset>`/`<Explanation>`/`<FlowDiagram>`; verify every `diagnose` uses `grading`, every `review` finding has `planted: true`, every `design` has `rubric` length ≥2, EN/RU lesson + practice counts match, Quiz ids match across langs, ≥1 `FlowDiagram` per lesson, no `StructureFigure`.
